...
mainActivity.refreshNewItemsMenu = function() {
	...
	var parentOrgKindID = justep.OpmUtils.isTreeRoot(dOrgTree.getID()) ? "root" : dOrgTree.getValue("sOrgKindID");
	...
	for (var i = 0; i < orgKindIDs.length; i++) {
		...
		} else if (justep.Array.indexOf(justep.Org.OrgKinds.getParents(orgKindID), parentOrgKindID) != -1) {
			newMenu.setItemEnabled(orgKindID);
		} else {
	...
	if (justep.Array.indexOf(justep.Org.OrgKinds.getParents(justep.Org.OrgKinds.ORGKIND_PERSONMEMBER), parentOrgKindID) != -1) {
...
mainActivity.gridOrgListSOrgKindIDRender = function(event) {
	...
	return "<img src='"
			...
			+ "' alt='" + justep.Org.OrgKinds.getLabel(orgKind) + "'/>";
};
...
mainActivity.newItemsMenuClick = function(event) {
	...
	var parentID = (justep.OpmUtils.isTreeRoot(dOrgTree.getID()) ? "" : dOrgTree.getID());
...
mainActivity.wdAssignPersonReceive = function(event) {
	var dOrgTree = justep.xbl("dOrgTree");
	var orgID = justep.OpmUtils.isTreeRoot(dOrgTree.getID()) ? null : dOrgTree.getID();
	...
	if (orgID) params.setString("orgID", orgID);
...
mainActivity.wdPersonDetailReceive = function(event) {
	if (event.data.openMode == "new") {
		justep.OpmUtils.showSuccess("新建人员成功。<br/><br/>“" + event.data.name + "”的初始密码为：<span style='color:#FF0000;font-size:16px;font-weight:bold;'>" + event.data.passwordClearText + "</span>。<br/><br/>请通知“" + event.data.name + "”尽快修改密码！");
	}
	mainActivity.resetTreeNode();
...
mainActivity.refreshListData = function(id) {
	...
	if (justep.OpmUtils.isTreeRoot(dOrgTree.getID())) {
		...
		if (isShowAllChildren)
			dOrgList.setFilter("parentFilter", "SA_OPOrg.sFID like '" + dOrgTree.getValue("sFID") + "/%'");
		else
			dOrgList.setFilter("parentFilter", "SA_OPOrg.sParent = '" + dOrgTree.getID() + "'");
	}
	
	var isOnlyShowMasterPsm = document.getElementById("cbOnlyShowMasterPsm").checked;
	if (isOnlyShowMasterPsm)
		dOrgList.setFilter("masterPsmFilter", "SA_OPOrg.sOrgKindID <> 'psm' or SA_OPOrg.sParent = SA_OPPerson.sMainOrgID");
	else
	...
	if (displayOrgKinds) {
		dOrgList.setFilter("orgKindsFilter", "SA_OPOrg.sOrgKindID in ('" + displayOrgKinds.split(",").join("','") + "')");
	} else
	...
	if (!!searchText)
		dOrgList.setFilter("searchFilter", justep.OpmUtils.getMultiLikeFilter("SA_OPOrg.sName,SA_OPOrg.sCode,SA_OPPerson.sSex,SA_OPPerson.sIDCard", searchText));
	else
...
mainActivity.initOrgKindsData = function() {
	...
	for (var i = 0; i < orgKindIDs.length; i++) {
		...
		dOrgKinds.newData({
			"index": i,
			"defaultValues": [{"id": orgKindID, "label": justep.Org.OrgKinds.getLabel(orgKindID)}]	
		});
	}
};
...
mainActivity.initNewOrgMenu = function() {
	...
	for (var i = 0; i < orgKindIDs.length; i++) {
		...
		var menuItem = new xforms.XFMenuItem(orgKindID, null, "新建" + label, false, null, i, null, justep.Org.OrgKinds.getImageURL(orgKindID), justep.Org.OrgKinds.getImageURL(orgKindID, false, true));
...
mainActivity.setComponentsStatus = function() {
	...
	justep.xbl("btnInsertMore").setDisabled(!justep.OpmUtils.isTreeRoot(dOrgTree.getID()) && (treeValidState == 0));
	...
	justep.xbl("menuButton1").setDisabled('itemEnable', !listRowID || listValidState == 1);
	justep.xbl("menuButton1").setDisabled('itemDisable', !listRowID || listValidState == 0);
	justep.xbl("menuButton1").setDisabled('itemLogicDelete', !listRowID);
	justep.xbl("menuButton1").setDisabled('itemResetPassword', !listRowID || listOrgKind != "psm");
	justep.xbl("menuButton1").setDisabled('itemChangeMainOrg', !listRowID || listOrgKind != "psm");
...
mainActivity.btnEnableClick = function(event) {
	...
	if (orgKind != "psm" || isMasterPsm || personValidState == 1) {
		justep.OpmUtils.showConfirm("确实要启用“" + name + "”吗？" + (orgKind != "psm" ? "<br/><br/>“启用”操作会同时启用选中组织的所有下级组织。" : ""), 
			function(e){
				var params = new justep.Request.ActionParam();
				...
				justep.Request.sendBizRequest2({
					...
					callback: function(callbackData) {
						callbackData.ignoreError = false;
						if (callbackData.state) {
							if (orgKind != "psm") mainActivity.resetTreeNode();
							mainActivity.refreshListData();
		...
		justep.OpmUtils.showConfirm("“" + name + "”" + "的主岗位已被禁用，在启动当前岗位时会同时启用主岗位。" + "<br/><br/>确实要启用“" + name + "”的主岗位和当前岗位吗？",
			function(e) {
				var params = new justep.Request.ActionParam();
...
mainActivity.btnDisableClick = function(event) {
	...
	justep.OpmUtils.showConfirm("确实要禁用“" + name + "”吗？" 
			...
			justep.Request.sendBizRequest2({
				...
				callback: function(callbackData) {
					...
					if (callbackData.state) {
						if (orgKind != "psm") mainActivity.resetTreeNode();
						mainActivity.refreshListData();
						justep.OpmUtils.showSuccess("禁用“" + name + "”成功。");
					}
				}
			});
		}
...
mainActivity.btnLogicDeleteClick = function(event) {
	...
	justep.OpmUtils.showConfirm("确实要删除“" + name	+ "”吗？" 
			...
			justep.Request.sendBizRequest2({
				...
				callback: function(callbackData) {
					...
					if (callbackData.state) {
						if (orgKind != "psm") mainActivity.resetTreeNode();
						mainActivity.refreshListData();
						justep.OpmUtils.showSuccess("删除“" + name + "”成功。");
					} 
				}
			});
		}
...
mainActivity.btnSortClick = function(event) {
	...
	justep.xbl("wdSortOrgs").open( {
		"parentID" : justep.OpmUtils.isTreeRoot(dOrgTree.getID()) ? null : dOrgTree.getID()
	});
};

mainActivity.wdSortOrgsReceive = function(event) {
	var dOrgTree = justep.xbl("dOrgTree");
	var parentID = justep.OpmUtils.isTreeRoot(dOrgTree.getID()) ? null : dOrgTree.getID();
	...
	if (parentID) params.setString("parentID", parentID);
...
mainActivity.btnMoveClick = function(event) {
	...
	justep.OpmUtils.showConfirm("确实要移动“" + name + "”吗？", 
		function(e) {
			justep.xbl("wdSelectMoveToOrg").open({
				"filter": "not (SA_OPOrg.sFID like '" + dOrgList.getValue("sFID") + "%')",
				"rootFilter": "",			
				"manageCodes": "",	
...
mainActivity.wdSelectMoveToOrgReceive = function(event) {
	...
	var orgKind = dOrgList.getValue("sOrgKindID");
	justep.OpmUtils.showConfirm("确实要移动“" + name + "”到“" + newParentName + "”下吗？",
		function(e) {
			var params = new justep.Request.ActionParam();
			params.setString("id", id);
			params.setInteger("version", version);
			params.setString("newParentID", justep.OpmUtils.isTreeRoot(newParentID) ? "" : newParentID);
			justep.Request.sendBizRequest2({
				dataType: "json",
...
mainActivity.wdChangeMainOrgReceive = function(event) {
	...
	justep.Request.sendBizRequest2({
		...
		callback: function(callbackData) {
			...
			if (callbackData.state) {
				mainActivity.refreshListData();
				justep.OpmUtils.showSuccess("设置“" + name + "”的主岗“" + newMainOrgFName + "”成功。");
			} 
		}
	});
};
...
mainActivity.btnResetPasswordClick = function(event) {
	...
	justep.OpmUtils.showConfirm("确实要重置“" + name + "”的密码吗？ ",
		function(e) {
			var params = new justep.Request.ActionParam();
			params.setString("id", personID);
			params.setInteger("version", personVersion);
			justep.Request.sendBizRequest2({
				...
				callback: function(callbackData) {
					...
					if (callbackData.state) {
						mainActivity.refreshListData();
						justep.OpmUtils.showSuccess("重置“" + name + "”的密码成功。<br/><br/>新密码是：<span style='color:#FF0000;font-size:16px;font-weight:bold;'>" + callbackData.response + "</span>。<br/><br/>请通知“" + name + "”尽快修改密码！");
					}
				}
			});
		}
...
mainActivity.getCurrentPsmFID = function() {
	return justep.Context.getCurrentOrgFID() + "/" + justep.Context.getCurrentPersonID() + "@" + justep.Context.getCurrentOrgID() + ".psm";
};
...
mainActivity.checkCurrentPerson = function(fullID) {
	return (mainActivity.getCurrentPsmFID().indexOf(fullID) == 0) || (mainActivity.getCurrentMainPsmFID().indexOf(fullID) == 0);
};
...
mainActivity.itemAuthorizeClick = function(event){
	var dOrgList = justep.xbl("dOrgList");
	var title = "权限 - " + dOrgList.getValue("sName") + "（" + dOrgList.getValue("sFName") + "）";
