	...
	FuncFrame.prototype.init = function(){
		...
		this.headEl = $('' +
			'<div class="fixed-head">' +
				'<div class="fun-head">' + 
					'<div class="fun-head-inner">' +
						'<div class="fun-head-bar">' +
							'<div class="fun-head-left">' +
								'<a class="home-btn">首页</a>' + 	
								'<span class="title">' +
								'</span>' +
							'</div>' +
							'<div class="fun-head-right">' +
								'<span class="user">' +
//									'<span class="name"></span>' +
//									'<span class="loginTime"></span>' +
								'</span>' +
								'<span>' +
								'<a class="close">关闭功能</a>' +
								'</span>' +
							'</div>' +
						'</div>' +
		...
		if(!justep.Portal.agentInfo)
			...
			$('.logout a', this.headEl).click(function(){
	...
	FuncFrame.prototype.resize = function(){
		if(this.activeId && this.cache[this.activeId]){
			...
			option.frame.height($(document).height() - 43 -3);//ie8 下如果不多减一些会出滚动条
		}
	};
