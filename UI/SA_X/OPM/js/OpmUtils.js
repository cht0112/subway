...
justep.OpmUtils.getOrgKindLabel = function(orgKind) {
	if (Object.prototype.toString.apply(orgKind) == "[object Array]" && orgKind.length > 0) {
		orgKind = orgKind[0].getValue();
	} else if (typeof orgKind != "string") {
		return "";
	}
	if (orgKind == "ogn")
		return "机构";
...
justep.OpmUtils.showError = function(message, title) {
	/**var msg = message.substring(0, message.indexOf('|'));
	if (!msg)
		msg = message;
	alert((!title ? "" : title + "\n\r\n\r") + msg);**/
...
justep.OpmUtils.isTreeRoot = function(rowID) {
	return rowID == dhtmlXGridObject.prototype.ROOT_FLAG;
};
...
justep.OpmUtils.getOrgImgURL = function(id, disable, isMasterPsm) {
	if ("psm" == id && isMasterPsm)
		...
		return justep.Resource.getOrgImgURL(id, disable);
};
