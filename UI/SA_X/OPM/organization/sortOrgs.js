...
sortOrgs.btnMoveTopClick = function(event) {
	...
	if (rowID)
		sortOrgs.getSortData().getStore().moveRowPro(0 - index, rowID);
};
sortOrgs.btnMoveUpClick = function(event) {
	...
	var rowID = store.getSelectedRowId();
	if (rowID)
		sortOrgs.getSortData().getStore().moveRowPro(-1, rowID);
};
...
sortOrgs.btnMoveBottomClick = function(event) {
	...
	if (rowID)
		sortOrgs.getSortData().getStore().moveRowPro(count - index - 1, rowID);
};
sortOrgs.receiverReceive = function(event) {
	var data = sortOrgs.getSortData();
	if (!event.data.parentID)
		data.setFilter("parentFilter", "SA_OPOrg.sParent is null");
	else
		data.setFilter("parentFilter", "SA_OPOrg.sParent = '" + event.data.parentID + "'");
	data.refreshData();
};
...
sortOrgs.gridSortOrgSOrgKindIDRender =
		function(event) {
			return "<img src='" + justep.Request.convertURL(justep.Resource.getOrgImgURL(event.value), true)
					+ "' alt='" + justep.OpmUtils.getOrgKindLabel(event.value) + "'/>";
		};
