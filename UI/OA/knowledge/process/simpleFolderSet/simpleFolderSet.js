...
function initFolderRootFun() {
	...
	if (flag == true) {
		var data = justep.xbl('dFolder');
		data.refreshData();
		var folder = data.getCurrentRowId();
		data.instance.store.expand(folder);
	} else {
		alert("初始化栏目树失败!");
	}
}
...
function displayCurrentFolderInfo() {
	...
	var folderFName = justep.xbl("dFolder").getValue("fFullName");
	if (folderID != 'public') {
		try {
...
function canNewNodeFun(data) {
	var dFolder = justep.xbl('dFolder');
	if (typeof(dFolder) == "string")
		dFolder = justep.xbl(dFolder);
	var pName = dFolder.getValue("fName");
	if (pName == "") {
		alert("父栏目名称不能为空!");
		return false;
	}
...
function deleteFolderFun(event) {
	...
	var id = grdFolder.getSelectedRowId();
	var parent = grdFolder.getParentId(id);
	if(parent == '0') {
		alert("根节点不允许删除!");
		return;
	}
