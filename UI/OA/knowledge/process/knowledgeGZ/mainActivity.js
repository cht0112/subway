...
knowledgeBaseActivity.trgCancelClick = function(event){
	...
	if(len==0){
		...
		var r = justep.Request.sendBizRequest(justep.Context.getCurrentProcess(),
				justep.Context.getCurrentActivity(), "cancelKnowledgeGZAction",
				param);
		if (!justep.Request.isBizSuccess(r)) {
...
mainActivity.trgZSKClick = function(event){
	 var process = justep.Context.getCurrentProcess();
	 var activity = justep.Context.getCurrentActivity();
	 var url = "/OA/knowledge/process/knowledgeBase/knowledgeBaseActivity.w"
	 + "?process=" + process + "&activity=" + activity;
	 justep.Portal.openWindow("知识中心", url);
};
