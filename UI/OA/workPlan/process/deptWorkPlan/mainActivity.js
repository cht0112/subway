...
function dataMainValueChanged(event) {
	 ...
	 * (event.column == 'fPlanYear' || event.column == 'fCBBMID' || event.column ==
	 * 'fCBBM' || event.column == 'fPlanWeek' || event.column == 'fPlanMonth')) {
	 * concatForTittle(); }
	 */
...
function concatForTittle() {
	...
	var title = "";
	if ("" != fPlanYear && "" != fPlanMonth && "" != fPlanWeek) {
		title = fPlanYear + "年" + fPlanMonth + "月" + fPlanWeek + " 工作计划";
	} else if ("" != fPlanYear && "" != fPlanMonth) {
		title = fPlanYear + "年" + fPlanMonth + "月" + " 工作计划";
	} else if ("" != fPlanYear) {
		title = fPlanYear + "年" + fPlanMonth + " 工作计划";
	}
