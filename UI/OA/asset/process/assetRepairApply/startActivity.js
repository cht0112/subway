...
function mdMainxforms_model_construct_done(event) {
	...
	if (sData1 == null || sData1 == undefined) {
		...
		data.filters.setFilter("flowFilter", "OA_AS_RepairApply='" + sData1
				+ "'");
		data.refreshData();
	}
}
var isFilter = "";
var dRepairApplyValueChanging = false;
function dRepairApplyValueChanged(event) {
	if (!dRepairApplyValueChanging) {
		dRepairApplyValueChanging = true;
		try {
			...
			dRepairApplyValueChanging = false;
		}
	}
}
...
function wDlgAssetCardReceive(obj) {
	...
	data.setValue("fRemainValue", obj.data.fRemainValue);
}
...
*/
startActivity.assetRepairApplyProcessBeforeAdvanceQuery = function(data){
	var dRepairApply = justep.xbl('dRepairApply');
	var fCode = dRepairApply.getValue("fCode");
	if (fCode == "") {
		data.cancel = true;
		alert('请选择需要维修的资产！');
	}
