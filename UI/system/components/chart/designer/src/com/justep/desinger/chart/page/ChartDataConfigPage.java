...
public class ChartDataConfigPage extends Composite implements IPropertyDialogPage {
	...
	public static String beforeOpen(PropertyItem propertyItem){
		String dataListStr = ((XuiElement)propertyItem.getOwerElement()).getParentElement().getParentElement().getParentElement().getOriginElement().getAttribute("data-list");
		...
		if(!temp.equals("")){
			...
			((XuiElement)propertyItem.getOwerElement()).getParentElement().getParentElement().getParentElement().getOriginElement().setAttribute("data-list", temp);
		}
		
		//验证当前DATA定义中 引用数据源是否正确可用 SQL 情况  设计时依赖于运行时数据库连接 
		dataListStr = ((XuiElement)propertyItem.getOwerElement()).getParentElement().getParentElement().getParentElement().getOriginElement().getAttribute("data-list");
				...
				if(did.equals(dataId)){	
					 if(component.indexOf(XBL_COMPONENT_REPORT_DATA)!= -1){
							...
							if(ksqlText != null && !ksqlText.equals("")){
								...
								String basePath = StudioConfig.getBizSrcPath();
	...
	public void setPropertyItem(PropertyItem propertyItem) {
		...
		String dataListStr = ((XuiElement)this.propertyItem.getOwerElement()).getParentElement().getParentElement().getParentElement().getOriginElement().getAttribute("data-list");
		...
		for(int i=0; i<dataXuiList.size(); i++){
			...
			for(String did : dids){
				if(did.equals(dataId)){	
					...
					if(component.indexOf(XBL_COMPONENT_BIZ_DATA)!= -1){
					...
					}else if(component.indexOf(XBL_COMPONENT_REPORT_DATA)!= -1){
							...
							if(actionE != null){
								...
								if(actionName != null && !"".equals(actionName) && "ksqlaction".equals(type)){
									...
									if(!defaultModelName.equals(dataModel)) 
										model = ModelParser.parseModel(StudioConfig.getBizSrcPath(), dataModel);
									if(ksqlText != null && !ksqlText.equals(""))
