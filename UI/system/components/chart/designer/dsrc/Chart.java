...
//	
	...
	public Map appendChart(XuiElement currentElement){
		...
		if(baseChartType.indexOf("pie")!= -1){
			MessageDialog.openInformation(StudioPlugin.getShell(), "提示", "饼图不支持混合图！");
			return null;
		}else if(baseChartType.indexOf("gantt")!= -1){
			MessageDialog.openInformation(StudioPlugin.getShell(), "提示", "甘特图不支持混合图！");
			return null;
		}else if(baseChartType.indexOf("radar")!= -1){
			MessageDialog.openInformation(StudioPlugin.getShell(), "提示", "雷达图不支持混合图！");
			return null;
		}else if(baseChartType.indexOf("thermometer-meter")!= -1){
			MessageDialog.openInformation(StudioPlugin.getShell(), "提示", "温度计不支持混合图！");
			return null;
		}
		...
		ChartTypeSelectDialog dlg = new ChartTypeSelectDialog(StudioPlugin.getShell(),baseChartType);
		dlg.setSize(750, 550);
		dlg.open();
		...
		if(rv == null || rv.equals("")){
			...
			XuiElement chartXuiE = currentElement.getXuiDataModel().addElement("chartSeries",seriesSetE, null);
	...
	private void render(XuiElement currentElement){
		...
		ps.put("id",currentElement.getDesignId());
		//{"x":4,"y":4,"w":491,"h":369}
		...
		params.put("id",currentElement.getDesignId());
