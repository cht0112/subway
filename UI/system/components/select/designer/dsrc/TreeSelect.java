...
public class TreeSelect extends BaseComponent {
		...
		if(count<=0){
			...
			if(""!=dataId && !independence){
				...
				for(Element colE : columns){
					if(0==i && "false".equalsIgnoreCase(colE.getAttribute("visible"))){
						this.addError(xuiElement, "treeSelect组件itemset下columns属性中的第一列visible不能定义为false");
						result = false;
					}
