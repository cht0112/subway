justep.WindowRunner = function(url,title,maximise,onSend,onReceive){
	...
	if (onSend && onSend != "") this.attachEvent("onSend", (typeof(onSend)=='string')?eval(onSend):onSend, this);
	if (onReceive && onReceive != "") this.attachEvent("onReceive", (typeof(onReceive)=='string')?eval(onReceive):onReceive, this);
};

justep.WindowRunner.prototype.open = function(data,url,title, process, activity, executor, executorContext){
	this.open2({'data':data, 'title':title, 'url':url, 'process':process, 'activity':activity, 'executor':executor, 'executorContext':executorContext});
};
...
justep.WindowRunner.prototype.open2 = function(options){
	...
	var tabid = justep.Portal.openWindow(this.title,this.convertURL(this.url),this.maximise,this.sendToFrame);
...
justep.WindowRunner.prototype.sendToFrame = function() {
	var frame = this;
	var wo = frame.windowRunner;
	if (frame.contentWindow && frame.contentWindow.justep && frame.contentWindow.justep.windowReceiver
			&& frame.contentWindow.justep.windowReceiver.windowReceive) {
		frame.contentWindow.justep.windowReceiver.windowParentObj = wo;
		var eventData = {source : wo, data : wo.sendData};
		...
		window.setTimeout(function(){ frame.contentWindow.justep.windowReceiver.windowReceive(eventData); },300);
	}			
};
...
justep.WindowRunner.prototype.convertURL = function(url){
	...
	if (hasProcess != true)
		params.push("process=" + (this.process?this.process:justep.Context.getCurrentProcess()));
	if (hasActivity != true)
		params.push("activity=" + (this.activity?this.activity:justep.Context.getCurrentActivity()));
	if (hasExecutor != true)
		params.push("executor=" + (this.executor?this.executor:justep.Context.getExecutor()));
	if (hasExecutorContext != true)
		params.push("executorContext=" + (this.executorContext?this.executorContext:justep.Context.getExecuteContext()));
...
justep.WindowRunner.prototype.dispose = function(){
	if(this.iframe)this.iframe.windowRunner = null;
