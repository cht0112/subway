justep.WindowFrame = function(id,url,autoLoad,onSend,onReceive,onInit,onInitFrame,options){
	...
	if (onSend && onSend != "") this.attachEvent("onSend", eval(onSend), this);
	if (onReceive && onReceive != "") this.attachEvent("onReceive", eval(onReceive), this);
	if (onInit && onInit != "") this.attachEvent("onInit", eval(onReceive), this);
	if (onInitFrame && onInitFrame != "") this.attachEvent("onInitFrame", eval(onInitFrame), this);
...
justep.WindowFrame.prototype.initFrame = function() {
	if($("#iframe-"+this.id).size()>0) return;
	this.$c.html("<iframe id=\"iframe-" + this.id + "\" src=\"about:blank\" frameBorder=\"0\" style=\"width:100%;height:100%;\" scrolling=\"auto\"/>");
	this.$iframe = $("#iframe-"+this.id);
	this.$iframe[0].windowFrame = this;
	
	if (this.checkEvent("onInitFrame")) {
		this.callEvent("onInitFrame", [{source : this, frame : this.$iframe[0]}]);
	}	
};

justep.WindowFrame.prototype.open = function(data,url, process, activity, executor, executorContext) {
	this.open2({'data':data, 'url':url, 'process':process, 'activity':activity, 'executor':executor, 'executorContext':executorContext});
};
...
justep.WindowFrame.prototype.sendToFrame = function() {
	...
	var b = false;
	try{//沉默跨域的url
		b = frame.contentWindow && frame.contentWindow.justep && frame.contentWindow.justep.windowReceiver
...
justep.WindowFrame.prototype.callURL = function(url) {
	if($("#iframe-"+this.id).size()==0) this.initFrame();
	this.$iframe.attr("src", "about:blank");
	var frame = this.$iframe[0];
	this.$iframe.unbind("load").bind("load",function(){window.setTimeout(function(){frame.windowFrame.sendToFrame();},10);});
	if(url) this.url = url;
	var urlA = justep.Request.setBizParams(justep.Request.convertURL(this.url),this.process,this.activity,this.executor,this.executorContext);
	var self = this;
...
justep.WindowFrame.prototype.setExecutorContext = function(executorContext) {
	if(executorContext) this.executorContext = executorContext;
};
...
justep.WindowFrame.prototype.onReceiveByMapping = function(node, store) {
	...
	if(Object.prototype.toString.apply(store) === '[object Array]'){
		...
		try{
			...
			var msg = new justep.Message(justep.Message.JUSTEP231067,"WindowFrame");
