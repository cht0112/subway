...
public class OrgDialogEditMappingPage extends Composite implements IPropertyDialogPage {
	...
	public OrgDialogEditMappingPage(Composite parent, int style) {
		super(parent, style);
...
//		dataset.addColumn("remark").setLength(100).setDataType("String").setEditable(false).setEditor("TextCellEditor");	
	...
	private void createContents(Composite container){
		...
		// wj:隐藏来源概念
		conceptCombo1.setVisible(false);
...
//		});		
			...
			public void run() {
				getDisplay().syncExec(new Runnable(){
					public void run() {
						...
						// wj:固定显示dOrgList的关系
						conceptCombo1.setText("dOrgList");
						concetpCombo1Selection();
						lbl.dispose();
						}
					}				
	...
	public void setPropertyItem(PropertyItem arg0) {
		...
		try {
			// wj:按照单选或多选设置url 
			String multi = xblElement.getProperyValue("multi-select");
			String url = ("true".equalsIgnoreCase(multi)) 
				? "/UI/system/components/orgDialog/dialogs/orgMultiSelect.w"
				: "/UI/system/components/orgDialog/dialogs/orgSingleSelect.w";
//			String url = xblElement.getProperyValue("url");
			if(!url.startsWith("/"+StudioPlugin.UI+"/")){
				url = "/"+StudioPlugin.UI+url;
			}
			String base = new File(new File(StudioConfig.getRootPath(url)).getParent()).toURL().toString();
			this.winOri = this.parseExtendWindow(base,url);
		} catch (Exception e) {
			e.printStackTrace();
		}	
	...
	public Document parseExtendWindow(String base,String windowURL) {
		try {
			File libFile = new File(
					StudioConfig.getWindowExtendEngineBasePath() + "/lib");
			...
			Class c = classLoader.loadClass("com.justep.ui.WindowExtendsEngine");
			Constructor constructor = c.getConstructor(String.class);
			Object object = constructor.newInstance(base);
			Object obj = obj = XuiDynJavaManager.executeMethod(object, "excuteAsW3cDoc", new String[] { windowURL });
			return (Document) obj;
		} catch (Exception ex) {
		}
		return null;
	...
	private void setTableDataByXML(XuiElement xblElement){
		...
		List<Element> el = XPathUtil.selectNodes(this.winOri, ".//*[local-name()='data'][@concept!='']");//[@concept!='']		
		for (Element e:el){
			conceptCombo1.add(e.getAttribute("id"));			
		}
		
		List<Element> ell = XPathUtil.selectNodes(this.winTar, ".//*[local-name()='data'][@concept!='']");//[@concept!='']		
		for (Element e:ell){
			conceptCombo2.add(e.getAttribute("id"));			
		}
	...
	private Map<String,String> getRelation(Document doc,String concept){
		...
		if(fromData==null){
		//TODO:原有通过doc取得数据的代码
		List<Element> el = XPathUtil.selectNodes(doc, ".//*[local-name()='data'][@id='"+concept+"']");			
		for (Element e:el){
			String dataId = e.getAttribute("id");
			if (dataId != null){
				String columns = e.getAttribute("columns");
				if(null!=columns && !columns.equals("")){
					String[] rels = columns.split(",");
					for(String rel : rels){
						
					}
				}else{
					String relation = e.getAttribute("relations");
					...
					if(moduleName!=null){
						...
						try {
/*							manager.reloadModule(StudioConfig.getBizSrcPath(), moduleName);
							ActionModel actModel = manager.queryActionModel(actionName);

							if(actModel==null){
								return null;
							}
							
							List<ActnRelation> atnRels = actModel.getRelations();
							for(ActnRelation atnRel : atnRels){ 
								String alias = atnRel.getAlias();
								if(relations != null && !CommonFun.contains(relations, alias, true)){
									continue;
								}
								...
								if (alias.toLowerCase().equals("version")){
									continue;
								}
								relationMap.put(alias,relLabel);
								//relationMap.put(alias, relLabel);
							}*/
							DataSet ds = XuiDataSourceManager.getRelationsOfActionMainConcept(moduleName,actionName);
							relationMap.put("rowid", "主键");
	...
	private String clearBrackets(String str){		
		int idx = str.indexOf("(");
		if (idx > 0){
	...
	private void checkAll(){		
		List<DataRecord> dl = dataset.getDataRecords();
		
		int count = dl.size();
		
		for(int i = count-1;i>=0;i--){
			String v = "";
			int no = ds1bak.findRecord("relation", dl.get(i).getString("to"));
			int no1 = ds1bak.findRecord("relation", dl.get(i).getString("from"));
			if (no==-1||no1==-1){
				v = "!未知关系";
			}else{
				v = remarkTargetConcept(ds1bak.getString(no,"concept"),conceptCombo2.getText());
				if (v.equals("")){
					for(int j = 0;j<i;j++){				
						v = remarkRepeatRelation(dl.get(i).getString("to"),dl.get(j).getString("to"));
					}				
				}
			}
			dl.get(i).setValue("remark", v);
		}
		
	}
	
	private String checkRepeatRelation(String str){
		
		List<DataRecord> drl = dataset.getDataRecords();
		String re = "";
		for (DataRecord dr:drl){			
			re = remarkRepeatRelation(dr.getString("to"),str);
			if (!re.equals("")) break;
		}
		return re;
	}	
	
	private String remarkRepeatRelation(String str1,String str2){
		return str1.equals(str2)?"!重复关系":"";		
	}

	private String remarkTargetConcept(String str1,String str2){
		return !str1.equals(str2)?"!不同概念":"";		
	}
	
	private String convertFidName(String str){
		if("fID".equals(str)) return "rowid";
		if("rowid".equals(str)) return "fID";
		return str;
	}
