...
public class ExcelFileListDatasource {
	...
	public List<String[]> getDatasource(PropertyItem propertyItem) {
		...
		if(ExtSpaceUIUtil.isInExtSpace(parentPath)){
			ExtFileSystemProvider p = new ExtFileSystemProvider(StudioConfig.getUISrcPath());
			...
			while (!fileList.isEmpty()) {
				...
				if (f1.isFile() && f1.getName().endsWith(".xls")) {
					String fileTempPath = f1.getPath().replace("\\", "/").replace("\\\\", "/");
