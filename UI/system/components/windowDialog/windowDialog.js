 ...
 */
justep.WindowDialog = function(id, url, title, modal, status, width, height,
		left, top, initEveryTimes, onSendToFrame, onReceiveFromFrame,
		onDialogInit, onDialogOpen, onDialogClose) {
	...
	this.isCall = false;
	this.isInited = false;
	this.dialog = new justep.Dialog(this.dlgId, title, modal, status, width,
			height, left, top);
	var self = this;
 ...
 * 打开dialog
 */
justep.WindowDialog.prototype.open = function(data, title, url, process, activity, executor, executorContext) {
...
justep.WindowDialog.prototype.ensure = function(store,noclose) {
	...
	if (frame.contentWindow && frame.contentWindow.xforms && frame.contentWindow.xforms.blur)
...
justep.WindowDialog.prototype.callURL = function() {
	if(this.url){
		var url = justep.Request.setBizParams(justep.Request.convertURL(this.url),this.process,this.activity,this.executor,this.executorContext);
		var id = this.frameId;
		var $iframe = $('#' + id);
		var frame = $iframe[0];
		$iframe.unbind("load").bind("load",function(){window.setTimeout(function(){frame.windowDialog.sendToFrame();},10);});
...
justep.WindowDialog.prototype.sendToFrame = function() {
	...
	}else if(b && frame.contentWindow && frame.contentWindow.windowReceive){//兼容旧版本
...
justep.WindowDialog.prototype.onReceiveByMapping = function(node, store) {
	...
	if(Object.prototype.toString.apply(store) === '[object Array]'){
		...
		try{
			...
			var msg = new justep.Message(justep.Message.JUSTEP231067,"WindowDialog");
