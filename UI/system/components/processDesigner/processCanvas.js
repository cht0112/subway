...
(function($) {
    ...
    $.extend(jQuery, {
        parseXml: function(xmlStr) {
            ...
            else if (typeof DOMParser != "undefined" && !!xmlStr) {
...
justep.graphics.ProcessRect = function(ownerCanvas,modelData){
    ...
    if(!modelData.xmlNode){
    	var root = $.parseXml('<'+tagName+' name="'+modelData["name"]+'" src="'+(modelData.src||'')+'"><label language="zh_CN"></label></'+tagName+'>');
...
justep.graphics.ProcessConnection.prototype = {
   updateConditionBranch:function(ifElseAct){     
	   ...
	   if(trueTarget){
		   $(xmlNode).appendXml('<true-output xmlns="http://www.justep.com/model" name="x" unit="'+trueTarget.inPort.owner.getName()+'"/>');
		   trueTarget.setText("是");
	   }
	   if(falseTarget){
		   $(xmlNode).appendXml('<false-output xmlns="http://www.justep.com/model" name="x" unit="'+falseTarget.inPort.owner.getName()+'"/>');
...
justep.graphics.ProcessCanvas.prototype = {
	...
	_mouseDown:function(e){
		 ...
		 //当前鼠标点在滚动条上直接返回
         if(!ownerFigure && this._isInScrollBar(e.clientX, e.clientY) || targetE.ownerConnection){  
        	 return;
         }
...
justep.graphics.ProcessWebCanvas.prototype.updatePropertyFromUI = function(figure,input){
	  ...
	  if(name == 'label'){
		 ...
		 this.setExecuteRuleProp(name,input.attr('checked'));
		  ...
		  $(name,xmlNode).remove();
		  if(value){ 
		     $(xmlNode).appendXml('<'+name+' xmlns="http://www.justep.com/model" name="x" unit="'+value+'"/>');
		  }
		  ...
		  for(var i = 0;i<trueFalseOutput.length;i++){
			  if(trueFalseOutput[i][0] != value){
				  ...
				  $(xmlNode).appendXml('<'+otherName+' xmlns="http://www.justep.com/model" name="x" unit="'+trueFalseOutput[i][0]+'"/>');
...
justep.graphics.ProcessWebCanvas.prototype.setExecuteRuleProp = function(propName,value,xmlNode){
	...
	if(propName == 'customized-enabled'){
		...
		if(excuteMode.length == 0){	
			relationValue.appendXml('<item xmlns="http://www.justep.com/model" relation="sExecuteMode">'+value+'</item>  '); 
		}else{
...
justep.graphics.ProcessWebCanvas.prototype.showPropertyDlg = function(self,data){  
	...
	html.push("<div class='content' style='height:400px;'><textarea style='border:0;width:646px;height:398px'></textarea></div>");
    ...
    for(var i = 0,l=nodes.length;i<l;i++){
    	...
    	if(type){
    		...
    		if(type == 'connection'){
    			...
    			processMainData[key] = {sourceNode:node.getAttribute("from"),targetNode:node.getAttribute("to"),xmlNode:node,name:key,src:key,type:type,LABEL:$(">label",node).text(),style:shapeStyle[key]};
    		}else{
    			processMainData[key] = {xmlNode:node,name:key,src:key,type:type,LABEL:$(">label",node).text(),style:shapeStyle[key]};
    		}
...
justep.graphics.ProcessWebCanvas.prototype.initToolbox=function(xml){ 
   ...
   if(itemsNode){
	   ...
	   for(var i = 0;i<items.length;i++){
		   ...
		   htmlBuf.push('<li name="dynItem"><a href="javascript:void(0);" label="'+label+'"  onfocus="this.blur();" class="'+cls+'" src='+activity+' title="'+activity+'">'+label+'</a></li>');
	   }
   ...
   $(".toolbox-items>li>a",p).each(function(idx){
	  ...
	  target.bind("mousedown",function(event){
		 ...
		 if(clsName == 'cancel-selected'){
			 ...
			 if(type){   
				 self.selectProcessItemType(type,false,{src:target.attr("src"),LABEL:target.attr("label")||'',type:type});
			 }
		 }
	 });
  });
         ...
         for(var i = 0;i<cons.length;i+=1){
         	...
         	if(con.outPort.owner == figure){
         		if(xmlNode.tagName != 'if-else-activity'){
         			$(xmlNode).appendXml('  <output xmlns="http://www.justep.com/model" name="x" unit="'+con.inPort.owner.getName()+'"/>\n');         			
         		}
         	}else{
         		hasInput = true;
         		$(xmlNode).appendXml('  <input xmlns="http://www.justep.com/model" name="x" unit="'+con.outPort.owner.getName()+'"/>\n');
         	}
         } 
         if(hasInput == false){ 
         	//start = modelData.name;
