 ...
 * 
 */
justep.Dialog = function(id,title,modal,status,width,height,left,top,onInit,onOpen,onClose){
	...
	if(typeof this.top=="string"){
		try{
			...
			var msg = new justep.Message(justep.Message.JUSTEP231047,"top",this.top);
	...
	if(typeof this.left=="string"){
		try{
			...
			var msg = new justep.Message(justep.Message.JUSTEP231047,"left",this.left);
			throw justep.Error.create(msg);
		}
	}
...
justep.Dialog.prototype.initDialog = function(){
	...
	if(!justep.Dialog.dialogs){
		...
		justep.Dialog.dialogs.setImagePath(justep.Request.convertURL("form/dhtmlx/" + "dhtmlxWindows/imgs/", true));
	}
	if(this.neighbor){
	...
	this.dlg = justep.Dialog.dialogs.createWindow(this.id+"-dlg-id", this.left, this.top, this.width, this.height);
...
//		this.dlg.button("minmax2").show(); 
	...
	if(!justep.Dialog.dialogs._fixedSize){
		...
		//ie6，ie7下有bug
		if(!justep.Browser.IE6 && !justep.Browser.IE7)	w.bind("resize", _r_size);
    }
	
	 //如果第一次，并且不是最大化，并且没有邻居 ，则显示在屏幕中央，自己计算（dhtmlx提供的方法有bug）
    if(!this.dlg._isMaximized && !this.neighbor && this.left==-1 && this.top==-1){
    	var b = $(document);
    	var w = $(window);
    	if(typeof this.dlg.w=="string"){
    		this.dlg.w = parseInt(this.dlg.w);
    	}
    	this.dlg.x = b.scrollLeft()+ (w.width()-this.dlg.w  -22)/2;
    	
    	if(typeof this.dlg.h=="string"){
    		this.dlg.h = parseInt(this.dlg.h);
    	}
   		this.dlg.y =  b.scrollTop()+ (w.height()-this.dlg.h - 4)/2;
   		
    	if(this.dlg.y<0)this.dlg.y=0;
    	if(this.dlg.x<0)this.dlg.x=0;
    ...
    if (this.checkEvent("onOpen")){
		this.callEvent("onOpen", [{source:this}]); 
	}
	justep.XBLObject.resize(this.id);
};

justep.Dialog.prototype.close = function(){
	if(this.status=="maximize")	$("#visible-element").show();
	if(!this.dlg||this.dlg.style.display=="none")return;
    this.dlg.hide();
    if(this.modal){
    	this.dlg.setModal(false); 
    }
    if (this.checkEvent("onClose")){
		this.callEvent("onClose", [{source:this}]); 
	}
};
...
justep.Dialog.prototype.setTitle = function(title) {
	this.title = title;
	if(this.dlg) this.dlg.setText(title);
};

justep.Dialog.prototype.setShowTitle = function(isShow) {
	this.isShowTitle = isShow;
	if(this.dlg) isShow?this.dlg.showHeader():this.dlg.hideHeader();
};
