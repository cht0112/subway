...
justep.AttachmentEditor2.prototype = {
	...
	initComponent : function() {
		...
		//TODO: this.uploaderDiv
		this.displayStyle = this.getAttribute("display-style", "list");
		var displayButtonsStr = this.getAttribute("display-buttons","upload:true;template:true;download:true;edit:true;delete:true;history:true;");
		...
		if (domAccess) {
			this.access = justep.doc.InnerUtils.accessEnum[domAccess] ? justep.doc.InnerUtils.accessEnum[domAccess]: domAccess;
		}
		...
		//TODO this.data
		this.autoCreateVersion = ("true" == this.getAttribute("autoCreateVersion") ? true : false);
		this.changeLog = {
			items : [],
	...
	_attachStoreEvent : function() {
		...
		if (data && (!this.eventAttached)) {
			...
			data.attachEvent(justep.XData.EVENT_INDEX_CHANGED, dataValueChangedCallback,
					this);
			data.attachEvent(justep.XData.EVENT_DATA_CHANGED, dataValueChangedCallback,
					this);
	...
	renderContainer : function() {
		...
		if($('#' + this.container).length == 0){
			var containerEle = $('<div></div>').attr('id',this.container).addClass('att_container');
			var containerHeadEle = $('<div></div>').addClass('att_head').attr('id',this.containerHead);
			if (this.buttons.upload) {
				var uploadEle = $('<span></span>').addClass('att_head_btn').addClass('att_upload_btn').attr('id',this.uploaderButton).text(new justep.Message(justep.Message.JUSTEP232501).getMessage());
			...
			if (this.buttons.template) {
				var teplEle = $('<a></a>').addClass('att_head_btn').attr('id',this.templateButton).attr('href','javascript:void(0);').text(new justep.Message(justep.Message.JUSTEP232500).getMessage()); 
				if (this.access % 512 >= 256) {
			...
			if (this.buttons["delete"]) {
				$('<th></th>').addClass('att_del_col').appendTo(attListTheadEle);
			}
			if (this.buttons.history) {
				$('<th></th>').addClass('att_his_col').appendTo(attListTheadEle);
			}
			attListTheadEle.prependTo(innerContainerEle);
			
			var containerBodyEle = $('<div></div>').attr("id",this.containerBody).addClass('att_body');
			$('<div></div>').addClass('att_scroll_area').append(innerContainerEle).appendTo(containerBodyEle);
	...
	renderContent : function(){
		...
		if(value){
			...
			if(this.limit !=-1 && data.length >= this.limit){
				this.containerBodyEle.removeClass('att_body').addClass('att_body_limited');
				this.containerHeadEle.removeClass('att_head').addClass('att_head_limited');
			}else{
				this.containerBodyEle.removeClass('att_body_limited').addClass('att_body');
				this.containerHeadEle.removeClass('att_head_limited').addClass('att_head');
			}
	...
	renderOne :function(data,i){
		var isOdd = i%2 == 0?true:false;
		var self = this;
		var docID = data.docID;
		var item =  self.getDataByID(docID);
		var attRowEle = $('<tr></tr>').addClass('att_body_row').addClass(isOdd?'att_body_row_odd':'att_body_row_ev');
		if ("docName") {
			var displayEle = $('<span></span>').addClass('att_body_btn att_name_btn').attr('name',this.id+'_docDisplay').text(data.docName);
			if (this.access % 4 >= 2 && item.fileID != ""){
				displayEle.bind('click',function(){
						self.browseDoc(docID,item.docName,item.fileID);
					});
			} else {
				displayEle.addClass('att_btn_dis');
			}
			$('<td></td>').append(displayEle).appendTo(attRowEle);
		}
		if ("docSize") {
			$('<td></td>').append($('<span></span>').addClass('att_body_btn').addClass('att_size_btn').attr('name',this.id+'_docSize').text(justep.doc.InnerUtils.formatSize(data.size)))
				.appendTo(attRowEle);
		}
		if (this.buttons.edit) {
			var editEle = $('<span></span>').attr('id',this.id+ '_' + docID +'_docEdit').addClass('att_body_btn att_edit_btn').attr('name',this.id+'_docEdit').text(new justep.Message(justep.Message.JUSTEP232504).getMessage());
			if (this.access % 1024 >= 512) {
				editEle.bind('click',function(){
						self.editDoc(docID,item.fileID,item.docName,item.docPath);
					});
			}else{
		...
		if (this.buttons.download) {
			var downEle = $('<span></span>').addClass('att_body_btn').addClass('att_down_btn')
			.attr('name',this.id+'_docDown').text(new justep.Message(justep.Message.JUSTEP232502).getMessage());
			if (this.access % 8 >= 4 && item.fileID != ""){
				downEle.bind('click',function(){
					self.downloadDoc(docID,item.fileID);
				});
			} else {
		...
		if (this.buttons["delete"]) {
			var deleteEle = $('<span></span>').addClass('att_body_btn').addClass('att_del_btn').attr('name',this.id+'_docDel').text(new justep.Message(justep.Message.JUSTEP232503).getMessage());
		...
		if (this.buttons.history) {
			var hisEle = $('<span></span>').addClass('att_body_btn').addClass('att_his_btn').attr('name',this.id+'_docHis').attr('href','javascript:void(0);').text(new justep.Message(justep.Message.JUSTEP232505).getMessage());
			if (this.access % 1024 >= 512) {
	...
	updateDoc : function(docID, fileID, docPath, docName, kind, size,
		...
		if (node) {
			...
			justep.doc.InnerUtils.modifyChangeLog(node, [ version, fileID, docVersionID,
			...
			var row = justep.doc.InnerUtils.queryDoc(docID, undefined, [ "version",
			...
			var billID = this.bindData.getCurrentRowId();
			justep.doc.InnerUtils.addChangeLog(this.changeLog, "edit", [ docID, version,
	...
	deleteDoc : function(docID, docName) {
		...
		if (confirm(new justep.Message(justep.Message.JUSTEP232506,docName).getMessage())) {
	...
	editMsDoc : function(docID, fileID, docName, docPath) {
		...
		OVP.isPrint = true;
		var currentNode = justep.doc.InnerUtils.evalChangeLog(this.changeLog, docID);
		if (currentNode != null) {
			var node;
			...
			if (cacheInfo.isLockSuccess == "success"
				...
				alert(new justep.Message(justep.Message.JUSTEP232055).getMessage());
				return;
			}
		}
		...
		var callback = function(event) {
			...
			if (data.type == "officeAction") {
				if (data.changes != 'W10=') {
					this.updateDoc(docID, fileID, docPath,
					...
					this.createVersion(docID, fileID, data.filename,
							docPath);
				} else {
					var currentNode = justep.doc.InnerUtils.evalChangeLog(
							this.changeLog, docID);
					if (currentNode == null
							&& this.lastOperation == "success") {
	...
	newFormTemplate : function() {
		...
		if (this.officeTemplates.length == 1) {
			...
			// var docServerInfo = justep.doc.InnerUtils.getDocServerByDocPath(cell.sDocPath);
			...
			var uploadDoc = function(event) {
				...
				if (data.type == "officeAction") {
					if (data.changes != 'W10=') {
						this.uploadDoc(data.filename, data.mediatype, data.size,
								data.cacheName, data.revisionCacheName,
								data.changes);
					}
				}else if(data.type == "officeLoaded"){
			...
			justep.doc.Dialog.openDocTemplateDialog(this.officeTemplates,afterSelectFun, this);
		} else {
	...
	commit : function() {
		var commitRows = justep.doc.InnerUtils.commitDoc(this.changeLog, this.rootPath,
				"commitAttachAction");
		if (commitRows) {
			...
			for ( var i = 0; i < items.length; i++) {
				...
				if ((item.userdata.recordState == 'new' || item.userdata.recordState == 'edit')
					...
					var data = this.bindData.getValue(this.relation,
							billID);
					if (!data) {
						continue;
					}
					...
					justep.doc.InnerUtils.updateChangeLog(this.changeLog, docID,
	...
	getDocLinkDefine : function() {
		var define = justep.doc.InnerUtils.queryDefine(this.process, this.activity,
				this.keyId, true);
		if (define) {
			if (define.rootPath) {
	...
	initHtml4Uploader : function() {
		...
		var completedCallback = function(docName, uploader, response) {
			...
			var cacheName = $(file).attr("file-name");
			var size = $(file).attr("fileSize");
			this.uploadDoc(docName, kind, size, cacheName);
		};
		var uploadParam = {
	...
	addEvents : function() {
		...
		this.__onOpenOfficeEditor = this.domNode.getAttribute("onOpenOfficeEditor") ? justep.Function
				.get(this.domNode.getAttribute("onOpenOfficeEditor"))
				: null;
		if (this.__onOpenOfficeEditor && typeof this.__onOpenOfficeEditor == "function") {
			this.attachEvent("onOpenOfficeEditor", this.__onOpenOfficeEditor, this);
		}
	...
	getDataByID :function(docID){
		...
		if(value){
			...
			throw justep.Error.create(new justep.Message(justep.Message.JUSTEP232061).getMessage());
		}
	},
	browseDoc : function(docID, docName, fileID) {
		...
		justep.doc.InnerUtils.browseDocByFileID(this.rootPath, docName, fileID, "last",
				"content", 'OpenOffice');
	},
	downloadDoc : function(docID, fileID) {
