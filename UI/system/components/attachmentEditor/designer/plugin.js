...
justep.design.AttachmentEditor = function(config){   
	justep.design.AttachmentEditor.superclass.constructor.call(this,config);
};

justep.extend(justep.design.AttachmentEditor, justep.design.Component,{
	 paintContent:function(xmlNode){
	 	...
	 	if(LayoutUtils.isCellLayout(this.getParentComponent())){
			...
			this.setProperty('class',"xui-attachmentEditor",false,true);
		}
	},
	
	
	setProperty:function(name,value,s,u){
		justep.design.AttachmentEditor.superclass.setProperty.call(this,name,value,s,u);
	...
	setDisplayButtons : function(value){
		...
		for (var item in buttons) {
			try {
				...
				if(element) element.style.display = buttons[item] ? "block" : "none";
			} finally {
				continue;
			}
		}
