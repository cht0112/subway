	...
	Diagram.prototype = {
		...
		setConfig:function(config){
			this.config = config;
			if(!this.config)
				this.config = {bgColor:'NULL',bound:{x:10,y:10,w:256,h:110}} // no effact!
		}
	}
	/**
	 * 波特图
	...
	Baud.prototype.draw = function(){
		if(this.data){
			...
			var firstBound = this.config.bound || {x:10,y:10,w:256,h:110};// EFFECTIVE
	...
	Baud.prototype.newConnection = function(one,two){
		var conn = new Justep.graphics.Connection({bgColor:'NULL',points:[{x:0,y:0},{x:12,y:12}]});
		if(one&&two){
	...
	Baud.prototype.layout = function(aheadBound,next){
		...
		for(var i=0;i<l;i++){
			...
			var data = this.find(refId);
			if(data){
				var bound = {x:aheadBound.x+aheadBound.w+20,
							 y:aheadBound.y+i*(aheadBound.w+aheadBound.y/2+20),
							 w:aheadBound.w,
							 h:aheadBound.h};
	...
	Baud.prototype.newBaudRectangle = function(recBound,isFirst){
		...
		var baud = new Justep.graphics.BaudRectangle({bgColor:bgColor,bound:recBound,filter:'alpha(opacity=80)'});			
			baud.addPort(new Justep.graphics.Port({type:'OutPort',direction:Justep.PortDirection.RIGHT,visible:false,calcuFun:function(ownerBound){
			...
			if(isFirst)
				return baud;
			baud.addPort(new Justep.graphics.Port({type:'InPort',direction:Justep.PortDirection.LEFT,visible:false,calcuFun:function(ownerBound){
			     var bound = this.bound;		     			     
