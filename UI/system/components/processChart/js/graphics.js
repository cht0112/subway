...
Justep.extend(Justep.graphics.Rectangle, Justep.graphics.Figure, {
	...
	paint:function(){
		if(!this.element){
		   ...
		   this.border = this.bgColor != 'NULL' ?this.bgColor:"black";
	...
	setText:function(text){
		...
		if(this.element){
			if(!this.textSpan){
				...
				setStyle(this.textSpan ,{fontFamily:'宋体',fontSize:'9pt',textAlign:'center',filter:"alpha(opacity=60)",position:"absolute"});
				this.canvas.container.appendChild(this.textSpan);
				this.textSpan.style.top = textBound.y+"px";
				this.textSpan.style.left = textBound.x+"px";
				//this.textSpan.style.height = textBound.h+"px";
...
//		var offsetW = document.getElementById("p1").offsetWidth +4;
		...
		tempDiv = null;
//		return {x:cx-offsetW/2+2,y:cy-(offsetH/3),w:offsetW,h:offsetH}
		if (window.ActiveXObject) {
			return {x:this.bound.x + (this.bound.w - offsetW) / 2 + 3,y: this.bound.y + (this.bound.h - offsetH) / 2 + 2,w:offsetW,h:offsetH}
		} else {  
			return {x:this.bound.x + (this.bound.w - offsetW) / 2,y: this.bound.y + (this.bound.h - offsetH) / 2,w:offsetW,h:offsetH}
		}
	},
...
//		var offsetW = document.getElementById("p1").offsetWidth +4;
		...
		return {x:cx-offsetW/2+2,y:cy-8,w:offsetW,h:20}
	},
	...
	dispose:function(autoDestroy){  
		...
		if(this.element){
			if(this.textSpan){
				this.canvas.container.removeChild(this.textSpan);
			}
			if(this.flagTextSpan){
	...
	setSelect:function(){
		...
		this.createBox(x-2+w,y+(h/2)-2,"e-resize",'e');
		this.createBox(x-2,y+(h/2)-2,"w-resize",'w');
		this.createBox(x-2+(w/2),y+h-2,"s-resize",'s');
		...
		if(this.boxes && this.boxes.length>0){  
		    ...
		    for(var i = 0;i<this.boxes.length;i++){
		       ...
		       switch(box.direction){
				  case 'e':
				     box.setBound({x:x+w,y:y+(h/2)});
				     break;
				  case 'w':
				     box.setBound({x:x,y:y+(h/2)});
				     break;
				  case 's':
				     box.setBound({x:x+(w/2),y:y+h});
				     break;
				  case 'n':
				     box.setBound({x:x+(w/2),y:y});
				     break;
		       	  case 'wn':  
		       	    box.setBound({x:x,y:y});
		       	    break;
		       	  case 'ne':
		       	    box.setBound({x:x+w,y:y});
		       	    break;
		       	  case 'sw':
		       	    box.setBound({x:x,y:y+h});
		       	    break;
		       	  case 'es':
		       	    box.setBound({x:x+w,y:y+h});
		       	    break;
		       }
		    }
		}
...
Justep.graphics.DojoRectangle = function(config) {
	Justep.graphics.DojoRectangle.superclass.constructor.call(this,Justep.apply(config,{type:config.type||'DojoRectangle'}));
}

Justep.extend(Justep.graphics.DojoRectangle, Justep.graphics.Rectangle, {
	paint:function(createMethod){
		if(!this.element){ 
			...
			this.element = this.canvas.surface[createMethod](points);
			if(this.visible){   
			   this.element.setFill(this.bgColor).setStroke(this.border);
			}
	...
	calcuPoints:function(){  
		var poinits = [];
		var x =this.bound.x,y=this.bound.y,w=this.bound.w,h=this.bound.h;
		poinits.push({x:x,y:y});
		poinits.push({x:x+w,y:y});
		poinits.push({x:x+w,y:y+h});
		poinits.push({x:x,y:y+h});
		poinits.push({x:x,y:y});
		return poinits;
	}
});
...
Justep.graphics.Triangular = function(config) {  
	...
	Justep.graphics.Rhombus.superclass.constructor.call(this,Justep.apply(config,{type:config.type||'Triangular',bound:{x:bound.x,y:bound.y,w:bound.w || 100,h:bound.h||35}}));
}

Justep.extend(Justep.graphics.Triangular, Justep.graphics.DojoRectangle, {
	direction:'d', /*三角形的方向,默认为项*/
	
   _calcuTextBound:function(text){
     	var bound = Justep.graphics.Triangular.superclass._calcuTextBound.call(this,text);
     	bound.y -= 8;
		return bound;
	},
	/**
...
Justep.graphics.Rhombus = function(config) {
	var bound = config.bound || {};
	Justep.graphics.Rhombus.superclass.constructor.call(this,Justep.apply(config,{type:config.type||'Rhombus',bound:{x:bound.x,y:bound.y,w:bound.w || 100,h:bound.h||35}}));
}

Justep.extend(Justep.graphics.Rhombus, Justep.graphics.DojoRectangle, {
	calcuPoints:function(){  
		var poinits = [];
		var x =this.bound.x,y=this.bound.y,w=this.bound.w,h=this.bound.h;
		poinits.push({x:x,y:y+h/2});
		poinits.push({x:x+w/2,y:y});
		poinits.push({x:x+w,y:y+h/2});
		poinits.push({x:x+w/2,y:y+h});
		poinits.push({x:x,y:y+h/2});
		return poinits;
	}
...
Justep.graphics.Hexagon = function(config) {
	var bound = config.bound || {};
	Justep.graphics.Hexagon.superclass.constructor.call(this,Justep.apply(config,{type:config.type||'Hexagon',bound:{x:bound.x,y:bound.y,w:bound.w || 100,h:bound.h||35}}));
}

Justep.extend(Justep.graphics.Hexagon, Justep.graphics.DojoRectangle, {
	calcuPoints:function(){  
		var poinits = [];
		var x =this.bound.x,y=this.bound.y,w=this.bound.w,h=this.bound.h;
		poinits.push({x:x,y:y+h/2});
		poinits.push({x:x+w/4,y:y});
		poinits.push({x:x+w*3/4,y:y});
		poinits.push({x:x+w,y:y+h/2});
		poinits.push({x:x+w*3/4,y:y+h});
		poinits.push({x:x+w/4,y:y+h});
		poinits.push({x:x,y:y+h/2});
		return poinits;
	}
});
...
Justep.graphics.Trapezoid = function(config) {
	var bound = config.bound || {};
	Justep.graphics.Trapezoid.superclass.constructor.call(this,Justep.apply(config,{type:config.type||'Trapezoid',bound:{x:bound.x,y:bound.y,w:bound.w || 100,h:bound.h||35}}));
}

Justep.extend(Justep.graphics.Trapezoid, Justep.graphics.DojoRectangle, {
	calcuPoints:function(){  
		var poinits = [];
		var x =this.bound.x,y=this.bound.y,w=this.bound.w,h=this.bound.h;
		poinits.push({x:x,y:y});
...
Justep.graphics.Circle = function(config) {
	var bound = config.bound || {};
	Justep.graphics.Circle.superclass.constructor.call(this,Justep.apply(config,{type:config.type||'Circle',bound:{x:bound.x,y:bound.y,w:bound.w || 40,h:bound.h||40}}));
}
...
Justep.extend(Justep.graphics.Circle, Justep.graphics.Rectangle, {
	 ...
	 */
	calcuPoints:function(){
		var poinits = {cx:this.bound.x+this.bound.w/2,cy:this.bound.y+this.bound.h/2,r:Math.max(this.bound.w,this.bound.h)/2};
		//this.bound = {x:poinits.cx-poinits.r,y:poinits.cy-poinits.r,w:poinits.r*2,h:2*poinits.r};
		return poinits;
	}
});
 ...
 * 端口方向。
 */
Justep.PortDirection = {UP:0,RIGHT:1,DOWN:2,LEFT:3};
...
Justep.graphics.Port = function(config) {
	var r = config.r || 4;
	Justep.graphics.Port.superclass.constructor.call(this,Justep.apply(config,{type:config.type||'Port',bgColor:[0,0,255,0.5],bound:{x:config.x,y:config.y,w:r*2,h:r*2}}));
	this.position = {x:this.bound.x,y:this.bound.y};
...
Justep.graphics.Line = function(config) {
    this.points = config.points || [{x:0,y:0},{x:0,y:0}];
	Justep.graphics.Line.superclass.constructor.call(this,Justep.apply(config,{type:config.type||'Line'}));
	this.boxes = []
...
Justep.extend(Justep.graphics.Line, Justep.graphics.Figure, {
	paint:function(){
		if(!this.element){
			...
			this.element = this.canvas.surface.createLine(points).setStroke({style:'ShortDash',color:this.border});
		    this.element.getEventSource().figure = this;
		}
	},
	dispose:function(){  
...
Justep.graphics.Connection = function(config) {
    this.points = config.points || [{x:0,y:0},{x:0,y:0}];
	Justep.graphics.Connection.superclass.constructor.call(this,Justep.apply(config,{type:config.type||'Connection'}));
	this.boxes = []
	this.text = config.text; 
	this.sn = (Justep.graphics._connCount++);
}

Justep.extend(Justep.graphics.Connection, Justep.graphics.Figure, {
	innerCount:0,
	paint:function(){
		if(!this.element){//alert("==="+JSON.stringify(this.points));
			...
			this.element.moveTo(this.points[0].x,this.points[0].y);
	...
	highLight:function(color){
		...
		if(color){
			this.element.setStroke(color);
			this.arrow.setStroke({color: color}); 
			this.arrow.setFill(color);
		}else{
			this.element.setStroke({color:'green'});
			this.arrow.setStroke({color: 'green'}); 
	...
	paintArrow:function(){
		if(this.inPort){
			var offset = 9;
			var p0={x:this.points[this.points.length-1].x,y:this.points[this.points.length-1].y},p1 = {},p2 = {},p4={x:p0.x,y:p0.y};
			...
			if(!this.arrow){ 
				this.arrow = this.canvas.surface.createPolyline([p0,p1,p2,p4]).setStroke("black").setFill("black");
...
//		if(bound){
			if(inPort == this.inPort){
				this.setPoint(this.points.length-1,{x:bound.x+bound.w/2,y:bound.y+bound.h/2}); 
				return;
			}//alert("======="+JSON.stringify(this.points));
//		}
		if(this.inPort){
			this.inPort.portType == 'Port';
			this.inPort.removeListener('positionChanged',this.positionChangedHandler,this);
			this.inPort.removeListener('dispose',this.dispose,this);
		...
		if(!hasCusMovePt){
		  var bound = inPort.bound;
		  this.points.push({x:bound.x+bound.w/2,y:bound.y+bound.h/2});
		...
		this.setShape(this.points);
		this.paintArrow();
		this.inPort.on('positionChanged',this.positionChangedHandler,this);
	...
	setOutPort:function(outPort){
		...
		if(outPort == this.outPort){
			this.setPoint(0,{x:bound.x+bound.w/2,y:bound.y+bound.h/2}); 
			return;
		}
		if(this.outPort){
			this.outPort.portType == 'Port';
			this.outPort.removeListener('positionChanged',this.positionChangedHandler,this);
			this.outPort.removeListener('dispose',this.dispose,this);
			this.outPort.addConnectionCount(-1);
		}
		outPort.portType = 'OutPort';
		this.outPort = outPort;
		this.setPoint(0,{x:bound.x+bound.w/2,y:bound.y+bound.h/2}); 
		this.outPort.on('positionChanged',this.positionChangedHandler,this);
...
/**
	...
	getTagLocationInPolyline : function() {
		...
		for (var i = 0; i < polyLinePoints.length - 1; i++) {
			...
			if (this.tagpointinpolyline) {
				resultP = this.getMidPoint(this.tagpointinpolyline.begin,this.tagpointinpolyline.end);
			}
			return resultP;
		}
	},
...
Justep.graphics.BaudTips = function(config) {
    ...
    Justep.graphics.BaudTips.superclass.constructor.call(this, Justep.apply(config, { type: config.type || 'BaudTips' }));
    this.data = config.data;
}
Justep.extend(Justep.graphics.BaudTips, Justep.graphics.Rectangle, {
	paint : function() {
        ...
        if(typeof executor =='string'){
	        ...
	        this.element.innerHTML = '<table width="100%" border="0" cellpadding="0" cellspacing="0" style="table-layout:fixed">' +
					'<tr><td' + style + '>标题:</td><td' + style0 + '>' + name + '</td></tr>' +
					'<tr><td' + style1 + '>执行者:</td><td' + style2 + '>' + executor + '</td></tr>' +
					//'<tr><td' + style1 + '>所属部门:</td><td' + style2 + '>' + executordepartment + '</td></tr>' +
					'<tr><td' + style1 + '>状态:</td><td' + style2 + '>' + status + '</td></tr>' +
					'<tr><td' + style1 + '>开始时间:</td><td' + style2 + '>' + formatDate(createtime) + '</td></tr>' +
					'<tr><td' + style1 + '>结束时间:</td><td' + style2 + '>' + formatDate(executetime) + '</td></tr>' +
					'</table>';
...
//        		var name = subTask.name || '';
		        ...
		        if(executetime!=''){
		        	executetime = '<tr><td' + style1 + '>结束时间:</td><td' + style2 + '>' + formatDate(executetime) + '</td></tr>';
		        }
		        var subTitleStyle = ' style="border-bottom:1px solid black;font:10pt;height:0px"';
		        //
		        var sCaption = '<tr height=\"5\"><td' + style + '></td><td' + subTitleStyle + '>' + '</td></tr>' ;
        		if(!hasTitle){
        			sCaption = '<tr><td' + style + '>标题:</td><td' + style0 + '>' + name + '</td></tr>' ; 
        		}
        		hasTitle = true;
		        ...
		        html += '<table width="100%" border="0" cellpadding="0" cellspacing="0" style="table-layout:fixed">' +
		        	//sCaption +
					//'<tr><td' + style + '>标题:</td><td' + style0 + '>' + name + '</td></tr>' +
					'<tr><td' + style1 + '>执行者:</td><td' + style2 + '>' + owner + '</td></tr>' +
					//'<tr><td' + style1 + '>所属部门:</td><td' + style2 + '>' + executordepartment + '</td></tr>' +
					'<tr><td' + style1 + '>状态:</td><td' + style2 + '>' + status + '</td></tr>' +
					'<tr><td' + style1 + '>开始时间:</td><td' + style2 + '>' + formatDate(createtime) + '</td></tr>' +
					executetime +
...
//		        };
        	...
        	for(var i = 0;i<this.element.children.length;i++){
       	   ...
       	   this.bound.h = this.element.scrollHeight || (parseInt(this.element.style.height)*executor.length);
    ...
    appendHeight: function (obj, delt) {
        ...
        if (!oldHeight || oldHeight == '') 
            ...
            oldHeight = oldHeight.substring(0, oldHeight.length - 2);
...
Justep.graphics.BaudRectangle = function(config){	
	Justep.graphics.BaudRectangle.superclass.constructor.call(this,Justep.apply(config,{type:config.type||'BaudRectangle'}));
	
}

Justep.extend(Justep.graphics.BaudRectangle, Justep.graphics.Rectangle, {
     paint: function () {
         Justep.graphics.BaudRectangle.superclass.paint.call(this);
         //		debugger;
         var name = this.data.name || '';
         var executor = this.data.executor || '';
         var executordepartment = this.data.executordepartment || '';
