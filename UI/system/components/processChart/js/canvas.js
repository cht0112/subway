...
Justep.graphics.BaseCanvas = function(config){
	...
	this.figures = {}; //存储图形对象
	this.scale = 1;
	this.addEvents('clickCreateFigure','remove','connectionChangePort','selectionchange','containerFocused'); 
}
Justep.extend(Justep.graphics.BaseCanvas, Justep.util.Observable, {
	...
	remove:function(figure){
		...
		if(type != 'Box' && type != 'Port' && type !='InPort' && type != 'OutPort'){
...
Justep.extend(Justep.graphics.Canvas, Justep.graphics.BaseCanvas, {

	actionType:{READY_CREATE:0,/*创建*/CREATE:1,/*改变大小*/RESIZE:2,/*移动*/MOVE:3,/*选择*/SELECT:4,READY_DRAG_SELECT:5,DRAG_SELECT:6,CHANGE_CONNECTION_SOURCE:7,CHANGE_CONNECTION_TARGET:8},//对图形的操作类型
...
//				}
	...
	getNearConnection:function(x,y){
		...
		for(var i =0 ,l=connections.length;i<l;i++){
			...
			for(var j = 0,l1=points.length-1;j<l1;j++){
				...
				if(containInLine_4CClick(x,y,{x1:p1.x,y1:p1.y,x2:p2.x,y2:p2.y})){
	...
	mouseDown : function() {
		...
		this.mouseDownPos = {x:event.clientX,y:event.clientY}; //鼠标按下的位置
        this.lastPosition = {x:event.clientX,y:event.clientY};
        if(this.currentMenu){
        	this.currentMenu.hide();
        	this.currentMenu = null;
        }
		var x = event.clientX-this.containerPos.x;// + this.scrollLeft;
		var y = event.clientY-this.containerPos.y;// + this.scrollTop; 
		...
		if (this.currentAction == this.actionType.READY_CREATE) {
			...
			if(this.currentType != 'Connection'){
				this.currentFigure = new Justep.graphics[this.currentType]({bound:{y:y,x:x}});
				this.add(this.currentFigure);
			}else{  
			    if((figure instanceof Justep.graphics.Port && (figure.portType!='InPort')) || this.fromPort){//OutPort
					var tempFigure = new Justep.graphics[this.currentType]({points:[{y:y,x:x},{y:y,x:x}]});
					this.add(tempFigure);
		...
		if (figure instanceof Justep.graphics.Box) {
			...
			if(this.currentFigure.type == 'Connection'){
				...
				if(idx == 0){
					...
					this.currentBox = figure;this.currentBox.element.style.display="none";
				}else if(idx>0 && idx == (this.currentFigure.boxes.length-1)){
		...
		if (figure) {
			...
			if(connection){
				this.currentFigure = connection;
				 this.lineIndex = connection.getPointIndex({x:x,y:y}); 
	...
	mouseMove:function(event){ 
		...
		if (this.currentAction == this.actionType.READY_CREATE || changeConnection) {	 
			
			if(this.currentType == 'Connection'  || changeConnection){
				var x = event.clientX-this.containerPos.x ;//+ (changeConnection?0:this.scrollLeft);
				var y = event.clientY-this.containerPos.y ;//+ (changeConnection?0:this.scrollTop); 
				var isMovePort = false;

				var figure = event.srcElement.figure;  
				if(changeConnection){
					this.currentFigure.setPoint(0,{x:x,y:y+5});
				}
				if(figure instanceof Justep.graphics.Port && (figure.portType!='InPort' )){//OutPort
					figure.setVisible(true);
					figure.setHighLight(true);
					this.fromPort = figure;
				}else if(figure instanceof Justep.graphics.Rectangle){

					var port = figure.getPortByPos(x,y,'OutPort');
					if(port){
						port.setHighLight(true);
						if(this.fromPort && this.fromPort.id != port.id){
							this.fromPort.setHighLight(false);
						}
						this.fromPort = port;
						isMovePort = true;
					} 
					if(this.moveOverFigure && this.moveOverFigure.id != figure.id){
				...
				if(this.fromPort && !(figure instanceof Justep.graphics.Port) && !isMovePort){ 
					  this.fromPort.setHighLight(false);
				}
			} 
			return;
		...
		if (this.currentAction == this.actionType.CREATE || changeConnection) {
			var x = event.clientX-this.containerPos.x ;//+ (changeConnection?0:this.scrollLeft);
			var y = event.clientY-this.containerPos.y ;//+ (changeConnection?0:this.scrollTop); 
			if(this.currentType == 'Connection' || changeConnection){
				var figure = event.srcElement.figure;  
				...
				if(this.currentFigure){
				    ...
				    this.currentFigure.setPoint(this.currentFigure.points.length-1,{x:x-5,y:y-5});
					if(figure instanceof Justep.graphics.Port && figure.portType !='OutPort'){//InPort
						if(changeConnection || figure.id != this.fromPort.id){
							figure.setVisible(true);
							figure.setHighLight(true);
							this.toPort = figure;
						}
					}else if(this.currentFigure && figure instanceof Justep.graphics.Rectangle){
						var x = event.clientX-this.containerPos.x ;//+ this.scrollLeft;
						var y = event.clientY-this.containerPos.y ;//+ this.scrollTop; 
						var port = figure.getPortByPos(x,y,'InPort');
						if(port){
							port.setHighLight(true);
							if(this.toPort && this.toPort.id != port.id){
								this.toPort.setHighLight(false);
							}
							this.toPort = port;
							isMovePort = true;
						...
						this.moveOverFigure = figure;
					}else if(figure && this.moveOverFigure && figure.id != this.currentFigure.id){
						this.moveOverFigure.setPortVisible(false);
					}
				} 
				if(this.toPort && !(figure instanceof Justep.graphics.Port) && !isMovePort){
					  this.toPort.setHighLight(false);
				}
	...
	dbclick:function(){
	    ...
	    }else if(figure instanceof Justep.graphics.Box && figure.owner instanceof Justep.graphics.Connection){
	...
	getAllConnections:function(sortBySN){
		...
		if(sortBySN){
		   conns = conns.sort(function(f1,f2){return (f1.sn > f2.sn)?-1:1});
		}
		return conns;
	},
	/**
				...
				var tempPoints = connections[jj].points;
					if(points[j].x == points[j+1].x){ //竖线	
						for(var n=0;n<tempPoints.length-1;n+=1){
							...
							(points[j].x>Math.min(tempPoints[n].x,tempPoints[n+1].x) && points[j].x<Math.max(tempPoints[n].x,tempPoints[n+1].x)) && 
							(tempPoints[n].y > minY && tempPoints[n].y < maxY)){
								points[j].attach = points[j].attach || [];
								var x = points[j].x,y=tempPoints[n].y; //交点
								var s = x+" "+(y-arcR)+" C"+(x-arcR)+" "+(y-arcR)+" "+(x-arcR)+" "+(y+arcR)+" "+x+" "+(y+arcR); // 弧形路径
								if(dir == Justep.PortDirection.UP){
									s = x+" "+(y+arcR)+" C"+(x-arcR)+" "+(y+arcR)+" "+(x-arcR)+" "+(y-arcR)+" "+x+" "+(y-arcR);  // 弧形路径
								} 
								points[j].attach.push({v:s,pos:y,x:x,y:y});
								hasInterPoint = true; 
							}
						}
						...
						for(var n=0;n<tempPoints.length-1;n+=1){ 
						    ...
						    (points[j].y > Math.min(tempPoints[n].y,tempPoints[n+1].y) && points[j].y < Math.max(tempPoints[n].y,tempPoints[n+1].y)) && 
						  	(tempPoints[n].x > minX && tempPoints[n].x < maxX)){  
						  		points[j].attach = points[j].attach || [];
								var x = tempPoints[n].x,y=points[j].y;  //交点
								var s = (x-arcR)+" "+y+" C"+(x-arcR)+" "+(y-arcR)+" "+(x+arcR)+" "+(y-arcR)+" "+(x+arcR)+" "+y;  // 弧形路径
								if(dir == Justep.PortDirection.LEFT){
									s = (x+arcR)+" "+y+" C"+(x+arcR)+" "+(y-arcR)+" "+(x-arcR)+" "+(y-arcR)+" "+(x-arcR)+" "+y;
								}
								points[j].attach.push({v:s,pos:x,x:x,y:y});
	                            hasInterPoint = true;
							}
						}
					}
    ...
    startDragSelect:function(event){  
       if(!this.dragLine){
		...
		var x = event.clientX-this.containerPos.x + this.scrollLeft ;
		var y = event.clientY-this.containerPos.y + this.scrollTop; 
    	this.dragLine = new Justep.graphics.DojoRectangle({bound:{x:x,y:y,w:18,h:18},bgColor:'NULL',border:{style:"ShortDash", width: 1,color:[0,0,0,0.3], cap: "round"}});
    ...
    getRegularBound : function(bound){
        ...
        return {x:minX,y:minY,w:Math.abs(bound.w),h:Math.abs(bound.h)};
    },
    ...
    endChangeConnection : function(figure,x,y) {
    	var portName = this.currentAction == this.actionType.CHANGE_CONNECTION_SOURCE?'outPort':'inPort';
    	var portType = this.currentAction == this.actionType.CHANGE_CONNECTION_SOURCE?'OutPort':'InPort';
    	var portSetMethod = this.currentAction == this.actionType.CHANGE_CONNECTION_SOURCE?'setOutPort':'setInPort';
    	
    	var fireChanged = false;
		this.currentFigure[portName].owner.setPortVisible(false);
		this.currentFigure.clearSelect();
		var newPort;// = this.currentAction == this.actionType.CHANGE_CONNECTION_SOURCE?this.fromPort:this.toPort;
		if (figure instanceof Justep.graphics.Port) {
			newPort = figure;
			figure = newPort.owner;
		} else if (figure instanceof Justep.graphics.Rectangle) {
			//var p = this.currentAction == this.actionType.CHANGE_CONNECTION_SOURCE?this.currentFigure.points[0]:this.currentFigure.points[this.currentFigure.points.length-1];
 
			newPort = this.moveOverFigure.getPortByPos(x, y, portType);
		}
		if (newPort) {
		...
		//if(fireChanged){ 
			this.fireEvent('connectionChangePort',this.currentFigure,newPort,oldPort);
		//}
	},
	...
	endCreate:function(figure){  
	    if(this.currentType == 'Connection'){
			if(this.currentFigure && figure && (figure instanceof Justep.graphics.Port || this.toPort) && figure.portType !='OutPort'){//InPort
	...
	startResize:function(event){
      if((Math.abs(event.clientX-this.mouseDownPos.x))>5 || (Math.abs(event.clientY-this.mouseDownPos.y))>5){
	...
	startMove:function(event){
		this.container.style.cursor = "move"; 
		 if(this.currentFigure instanceof Justep.graphics.Connection){ 
		      ...
		      if(this.lineIndex>0 && this.lineIndex<this.currentFigure.points.length-2){ 
		          var points = this.currentFigure.points;
		      	  this.ghostLine = new Justep.graphics.Line({border:'blue',points:[Justep.apply({},points[this.lineIndex]),Justep.apply({},points[this.lineIndex+1])]});
	...
	createGhostFigures:function(){ 
		for(var p in this.selections){ 
			...
			if(figure instanceof Justep.graphics.BaudRectangle){
				var cloneFigure = figure.cloneFigure({border:'NULL',bgColor:'#DDEEFF'});
				cloneFigure.owner = figure;
				this.ghostFigures[cloneFigure.id] = cloneFigure;
			}else if(figure instanceof Justep.graphics.Rectangle){
				var cloneFigure = figure.cloneFigure({border:'NULL',bgColor:[0,123,255,0.1]});
				cloneFigure.owner = figure;
				this.ghostFigures[cloneFigure.id] = cloneFigure;
			}
		}
	...
	clearGhostFigures:function(){
		...
		for(var p in this.ghostFigures){
			...
			if(owner.type == 'Circle'){
				var poinits = {cx:bound.x+bound.w/2,cy:bound.y+bound.h/2,r:Math.max(bound.w,bound.h)/2};
				bound = {x:poinits.cx-poinits.r,y:poinits.cy-poinits.r,w:poinits.r*2,h:2*poinits.r};
			}
			contents.push({figureId:owner.id,value:Justep.apply({},owner.bound)});
        ...
        if(this.ghostLine){  
        	var points = this.currentFigure.points;
        	contents.push({figureId:this.currentFigure.id,value:Justep.apply([],points)});
        	points[this.lineIndex] = Justep.apply(this.ghostLine.points[0]);
        	points[this.lineIndex+1] = Justep.apply(this.ghostLine.points[1]);
	...
	installListener:function(){
		...
		this.on(this.container, "ondblclick", this.dbclick, this);
		this.on(this.container,'onmousemove',this.mouseMove,this);
		this.on(this.container,'onkeydown', this.keyDown, this);
		this.on(this.container.parentNode,'onscroll',this.containerScroll,this);
        this.on(this.container,'onscroll',this.containerScroll,this); //zmh
        
	    dojo.connect(this.container, "ondragstart",   dojo, "stopEvent");
	    dojo.connect(this.container, "onselectstart", dojo, "stopEvent");
	   
	    this.on(document,'onmouseup',this.documentMouseUp,this);
	   
	    this.on(this.container, "onclick", this.click, this);
	    this.on(this.container, "onmouseover", this.mouseover, this);
	    this.on(this.container, "onmouseout", this.mouseout, this);
	}
});