...
function eXcell_attachmentEditor(cell) {
		...
		try{
			if(val =="" || val==null){  
				...
				for(i=0;i<docs.length;i++){
					...
					item.push("\" onclick=\"justep.doc.InnerUtils.browseDocByFileID($(this).attr('docPath'), $.trim(this.innerHTML.replace(';','')),$(this).attr('fileID'));" +
...
eXcell_attachmentEditor.prototype = new eXcell();

function eXcell_attachmentEditor2(cell){
	this.init(cell);
	this.AlwaysShowButton = false;
}

eXcell_attachmentEditor2.prototype = new eXcell_attachmentEditor();
