...
function queryFilterPatternData(data) {
	...
	param.setString("columns", "version,sName,sPerson,sProcess,sActivity,sInstance,sContent,sOrderNum,SA_FilterPattern");
	...
	return justep.Request.sendBizRequest("/SA/filterPattern/filterPatternProcess", "mainActivity", "queryFilterPatternAction", param, translateParam, null, true);
}
...
function showFilterPatternMenu(data, menuId, controlId, dialogId) {
	...
	if (menu.menu.$needLoad) {
		...
		var ii = 0;
		if(dialogId){
			var manageItem = new xforms.XFMenuItem("zdycx", null, (new justep.Message(justep.Message.JUSTEP231035)).getMessage());
			manageItem.pos = ii++;
		...
		var manageItem = new xforms.XFMenuItem("qkdqgl", null, (new justep.Message(justep.Message.JUSTEP231036)).getMessage());
		manageItem.pos = count;
		menu.addItem(manageItem);		
		menu.menu.addNewSeparator("qkdqgl");

		var manageItem = new xforms.XFMenuItem("glglms", null, (new justep.Message(justep.Message.JUSTEP231037)).getMessage());
