...
processDialog.refreshLayout = function() {
	if (processDialog.inputData != null) {
		...
		// 计算内容区域高度
		var contentHeight = $(window).height() - messageHeight - activitiesHeight - noticesHeight - $("#divContentTop").outerHeight(true) - $("#divBottom").outerHeight(true) - 2;
		$("#textareaContent").find("textarea").height(contentHeight - 2);
		$("#divContent").height(contentHeight);
		
		// 兼容pad
		$("#textareaContent").find("textarea").width($("#divContent").width() - 18);
	}
};
...
processDialog.windowReceiverReceive = function(event){
	...
	processDialog.inputData.processControl.loadFromJson(processDialog.inputData.data);
	
	// 初始化环节数据
	var dActivities = justep.xbl("dActivities");
	var activitiesRows = justep.ProcessDialog.Utils.activitiesToRows(processDialog.inputData.processControl.getActivities());
	dActivities.loadJson(activitiesRows);
	
	// 初始化通知数据
	var dNotices = justep.xbl("dNotices");
	var noticesRows = justep.ProcessDialog.Utils.noticeItemsToRows(processDialog.inputData.processControl.getNoticeItems());
	...
	if (gridActivities.getRowsNum() > 0) {
		processDialog.setActivityCheckStatus(gridActivities, gridActivities.getRowId(0), true, true, true, true);
	}
...
processDialog.wdSelectExecutorsReceive = function(event){
	...
	if (event.data.type == "to") {
		item = processDialog.inputData.processControl.getToItemByID(event.data.itemID);
		data = justep.xbl("dActivities");
	} else if (event.data.type == "notice") {
		item = processDialog.inputData.processControl.getNoticeItemByID(event.data.itemID);
		data = justep.xbl("dNotices");
	}
	item.clearExecutors();
	for (var i = 0; i < event.data.selected.length; i++) {
		item.addExecutor(event.data.selected[i].fid, event.data.selected[i].fname, event.data.selected[i].responsible);
	}
	var names = justep.ProcessDialog.Utils.getOrgUnitsNames(item.getExecutors());
	data.setValue("executorNames", names, event.data.itemID);

	// 依据是否选择执行者，重置选中状态
	if (event.data.type == "to") {
		var gridActivities = justep.xbl("gridActivities").grid;
		processDialog.setActivityCheckStatus(gridActivities, event.data.itemID, names != "", true, true, false);
	} else {
...
processDialog.btnExecuteClick = function(event){
	...
	if (processDialog.inputData.processControl.getToItems().length > 0) {
		...
		for (var i = 0; i < gridActivities.getRowsNum(); i++) {
			...
			if (name != "xor" && name != "and" && gridActivities.isCheckedRow_treegrid(id)) {
				var toItem = processDialog.inputData.processControl.getToItemByID(id);
	...
	if (processDialog.inputData.processControl.getNoticeItems().length > 0) {
		...
		for (var i = 0; i < gridNotices.getRowsNum(); i++) {
			...
			if (gridNotices.getValueByName("check", i) == 1) {
				var noticeItem = processDialog.inputData.processControl.getNoticeItemByID(id);
		...
		processDialog.inputData.processControl.setSelectedNoticeItems(noticeItemIDs);
	}
...
processDialog.setAdvancedOptionOfTo = function(activityID){
	var toItem = processDialog.inputData.processControl.getToItemByID(activityID);
...
processDialog.setAdvancedOptionOfNotice = function(noticeID){
	var noticeItem = processDialog.inputData.processControl.getNoticeItemByID(noticeID);
...
processDialog.gridActivities_advancedOptionRender = function(event){
	...
	var toItem = processDialog.inputData.processControl.getToItemByID(activityID);
	if (toItem != null && !toItem.isEnd()) {
		return "<label class='link' onclick='processDialog.setAdvancedOptionOfTo(\"" + activityID + "\")'>高级选项</label>";
	}
};
...
processDialog.gridActivities_executorNamesRender = function(event){
	...
	if (toItem != null && !toItem.isEnd()) {
		if (executorNames == "") {
			return "<label class='link" + (checked ? "" : "_no") + "_checked' onclick='processDialog.selectExecutors(\"to\", \"" +  itemID + "\")'>选择办理人</label>";
		} else {
			return "<label class='link" + (checked ? "" : "_no") + "_checked' onclick='processDialog.selectExecutors(\"to\", \"" +  itemID + "\")'>" + executorNames + "</label>";
		}
	}
};
...
processDialog.gridNotices_advancedOptionRender = function(event){
	...
	var noticeItem = processDialog.inputData.processControl.getNoticeItemByID(noticeID);
	return "<label class='link' onclick='processDialog.setAdvancedOptionOfNotice(\"" + noticeID + "\")'>高级选项</label>";
};
...
processDialog.gridNotices_executorNamesRender = function(event){
	...
	if (event.value == "") {
		return "<label class='link" + (checked ? "" : "_no") + "_checked' onclick='processDialog.selectExecutors(\"notice\", \"" +  itemID + "\")'>选择接收人</label>";
	} else {
		return "<label class='link" + (checked ? "" : "_no") + "_checked' onclick='processDialog.selectExecutors(\"notice\", \"" +  itemID + "\")'>" + event.value + "</label>";
	}
};
	...
	gridNotices.filterBy(0, function(data) {
		...
		for (var i = 0; i < gridActivities.getRowsNum(); i++) {
			if (data == gridActivities.getValueByName("name", i)) {
				return gridActivities.isCheckedRow_treegrid(gridActivities.getRowId(i));
			}
		}
		return true;
	});
...
processDialog.gridActivitiesRowChecked = function(event){
	processDialog.setActivityCheckStatus(event.grid, event.rowID, event.checked, true, true, true);
	processDialog.refreshNoticesFilter();
};

// 设置环节选择
processDialog.setActivityCheckStatus = function(grid, selfID, checked, up, down, forced) {
	if (forced || grid.isCheckedRow_treegrid(selfID) != checked) {
		// 设置自身
		grid.setItemChecked_treegrid(selfID, checked, false);
		grid.setValueById(selfID, "executorNames", grid.getValueById(selfID, "executorNames"));
		if (down) {
			// 向下递归
...
processDialog.setActivityParentCheckStatus = function(grid, selfID, checked) {
	...
	if (parentName == "and") {
		...
		for (var i = 0; i < subIDs.length; i++) {
			var subID = subIDs[i];
			if (subID != selfID) {
				processDialog.setActivityCheckStatus(grid, subID, checked, false, true, false);
			}
		}
		...
		for (var i = 0; i < subIDs.length; i++) {
			var subID = subIDs[i];
			if (subID != selfID) {
				processDialog.setActivityCheckStatus(grid, subID, false, false, true, false);
			}
		}
	}
	
	// 向上递归
	processDialog.setActivityCheckStatus(grid, parentID, checked, true, false, false);
};
...
processDialog.setActivityChildrenCheckStatus = function(grid, selfID, checked) {
	...
	if (name == "and") {
		...
		if (checked) {
			processDialog.setActivityCheckStatus(grid, subIDs[0], checked, false, true, false);
		} else {
			for (var i = 0; i < subIDs.length; i++) {
				var subID = subIDs[i];
				processDialog.setActivityCheckStatus(grid, subID, checked, false, true, false);
...
processDialog.dNoticesValueChanged = function(event){
	if (event.column == "check") {
		var gridNotices = justep.xbl("gridNotices").grid;
		gridNotices.setValueById(event.rowID, "executorNames", gridNotices.getValueById(event.rowID, "executorNames"));
	}
};
