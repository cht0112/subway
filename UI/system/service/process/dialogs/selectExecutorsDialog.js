...
selectExecutorsDialog.refreshLayout = function() {
	...
	justep.xbl("borderLayout1").leftEl.attr("size", Math.round($(window).width() / 2) - 50);
};
...
对话框入口
	入参格式
	{
		range: 		执行者范围（OrgUnit数组），range为["/"]表明所有的组织
		selected: 	已选中执行者（OrgUnit数组）
		orgKinds: 	执行者类型（组织结构节点类型）
		cascade: 	级联选择的初始值
	}
***********************************************************************************/
...
selectExecutorsDialog.windowReceiverReceive = function(event){
	...
	if (!selectExecutorsDialog.inputData.orgKinds) {
		selectExecutorsDialog.inputData.orgKinds = justep.Org.OrgKinds.getRealIDs().join(",");
	}
	selectExecutorsDialog.visibleOrgKinds = selectExecutorsDialog.getVisibleOrgKinds();

	$("#inputSearch").val("");
	$("#cbCascade").attr("checked", selectExecutorsDialog.inputData.cascade == true);  
...
selectExecutorsDialog.initOrgTree = function() {
	...
	if (range.length == 1 && range[0].fid == "/") {
		...
		for (var i = 0; i < range.length; i++) {
			...
			if (("," + selectExecutorsDialog.visibleOrgKinds + ",").indexOf("," + orgKind + ",") != -1) {
				orgUnits.push({fid: range[i].fid, fname: range[i].fname});
			}
		}
		var orgUnitsTree = justep.ProcessDialog.Utils.orgUnitsListToTree(orgUnits);
		dOrgTree.loadJson(justep.ProcessDialog.Utils.orgUnitsToRows(orgUnitsTree));
	}
	dOrgTree.setIndex(0);
};
...
selectExecutorsDialog.canSelect = function(grid, id) {
	...
	return !virtual && (("," + selectExecutorsDialog.inputData.orgKinds + ",").indexOf("," + orgKind + ",") != -1);
};
...
selectExecutorsDialog.createSearchFilter = function(searchText, relations) {
	...
	for (var i = 0; i < relations.length; i++) {
		filter.push("(upper(" + relations[i] + ") like '%" + searchText + "%')");
	};
...
selectExecutorsDialog.searchOrgList = function() {
	...
	if (searchText != "") {
		...
		var searchFilter = selectExecutorsDialog.createSearchFilter(searchText, ["SA_OPOrg.sName", "SA_OPOrg.sCode"]);
...
selectExecutorsDialog.getCurrentGrid = function() {
	return (selectExecutorsDialog.currentMode == "tree") ? justep.xbl("gridOrgTree") : justep.xbl("gridOrgList");
};

// 按照当前选择模式, 获取选择data
selectExecutorsDialog.getCurrentData = function() {
	return (selectExecutorsDialog.currentMode == "tree") ? justep.xbl("dOrgTree") : justep.xbl("dOrgList");
};
...
selectExecutorsDialog.isSelected = function(selectedFullIDs, id) {
	for (var i = 0; i < selectedFullIDs.length; i++) {
		...
		if (justep.Org.OrgUtils.getOrgID(fullID) == id) {
			return 1;
		} else if (selectExecutorsDialog.currentMode == "tree" && fullID.indexOf("/" + id + ".") != -1) {
			return 0.5;
		} 
	}
	return 0;
...
selectExecutorsDialog.refreshGridChecked = function(ids) {
	...
	for (var o in ids) {
		...
		if (grid.isRowRendered(id)) {
			...
			var selected = selectExecutorsDialog.isSelected(selectedFullIDs, realOrgID) > 0;
...
selectExecutorsDialog.deleteSelectedItems = function(ids) {
	...
	for (var j = fullIDs.length - 1; j >= 0; j--) {
		...
		if (justep.Array.indexOf(ids, fullID) != -1 || justep.Array.indexOf(ids, id) != -1) {
	...
	if (ids.length >= selectExecutorsDialog._limitOfDirectDeleteRows) {
		...
		gridSelected.loadData(null, selectExecutorsDialog.orgUnitsToSelectedRows(orgUnits), null, false, false, "json");
	}
	...
	if (selectExecutorsDialog.currentMode == "list" || !cascade) {
		...
		if (ids.length != 0) {
			...
			for (var i = 0; i < ids.length; i++) {
				...
				filter = justep.ProcessDialog.Utils.joinFilter("OR", filter, "sFID like '" + fullID + "%'");
			}
...
selectExecutorsDialog.btnOkClick = function(event){
	var gridSelected = justep.xbl("gridSelected").getDhtmlxGrid();
	selectExecutorsDialog.inputData.selected = selectExecutorsDialog.getSelectedOrgUnits();
...
selectExecutorsDialog.loadOrgCommonGroup = function() {
	...
	var filter = justep.ProcessDialog.Utils.joinFilter("and", orgKindsFilter.join("or"), rangeFilter.join("or"));
