...
public class ProcessDialogOrgChildrenQ implements JProcessor {

	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		Document instance = NetUtils.getInstance(request);
		Element e = (Element)instance.selectSingleNode("/form/data/data-id");
		...
		if (ar.isSuccess()){
			...
			for (Object obj : orgs){
				...
				jsonData += "data_id:\"" + parent + "\",";
				jsonData += "rows:[]";
				jsonData += "}";
			}
			
			Element root = result.addElement("root");
			root.setText(jsonData);
		}
