...
justep.doc._read_file_type2 = ".doc,.docx,.xls,.xlsx,.ppt,.pptx,.mpp,.vsd,.txt,.text,.jsp,.java,.html ,.htm,.xml,.css,.rtf,.wml,.jpg,.jpeg,.jpe,.png,.gif,.tiff,.tif,.svg,.svgz,.svg,.pdf,.wrl,.smil,.js,.vbs,.rdf,.odt,.ott,.uof";
/*是否显示菜单*/
...
justep.doc.Utils.getDocUrl = function(docIDorDocPath,urlType,versionID,partType,resultID){
		...
		callback: function(result) {
			if (result.state) {
				...
				if (docUrl.indexOf("uploadDoc.j") != -1 && docUrl.indexOf("#") == -1) {
					docUrl = window.location.protocol + "//" + window.location.host + docUrl;
				}
...
justep.doc.Utils.bindUploader = function(params){
	...
	if(params.completeCallBack){
		...
		params.completeCallBack = function(docName,uploader,response){
			...
			originComplateCB.call(this,docID,docName,docPath,fileID,kind,size,uploader);
		};
	}
...
justep.doc.Utils.browseDoc = function(docID,versionID,partType){
	var row = justep.doc.InnerUtils.queryDoc(docID, undefined, ["sDocPath","sDocName","sFileID"],undefined,undefined,"single");
	var docName = row.sDocName.value;
	var fileID = row.sFileID.value;
	var docPath = row.sDocPath.value;
	justep.doc.InnerUtils.browseDocByFileID(docPath, docName, fileID, versionID, partType);
};

justep.doc.Utils.downloadDoc = function(docID,versionID,partType){
	var row = justep.doc.InnerUtils.queryDoc(docID, undefined, ["sDocPath","sFileID"],undefined,undefined,"single");
	var fileID =  row.sFileID.value;
	var docPath =  row.sDocPath.value;
	justep.doc.InnerUtils.downloadDocByFileID(docPath, fileID, versionID, partType);
};
...
justep.doc.Utils.getOfficeViewer = function(id){
	...
	$('<div></div>').attr('id',ocxID).css({'width':'100%','height':'100%'}).appendTo($('#'+id));
...
justep.doc.OfficeViewer = (function(){
	if (!this.$OV2) this.$OV2 =  function (id) {
		var element = document.getElementById(id);
		if (element.OV2_OfficeViewer_extended) return element;
	...
	if (!this.OV2) this.OV2 = (function() {
		...
		function tryThese(source) {
			for (var i = OV2.isFunction(source) ? 0 : 1, l = arguments.length; i < l; i++) {
				try {
					return arguments[i](OV2.isFunction(source) ? undefined : source);
				} catch(e) {}
			}
			return null;
		}
		...
		function isJSON(str) {
			return OV2.isString(str) && Boolean(str.length) && 
		...
		var OfficeViewer = (function() {
			...
			function CreateOfficeViewer(width, height) {
				...
				if (!this.IsOfficeViewer()) {
					...
					if(OV2.IsIE()){
						this.outerHTML = OV2.substitute(OV2.getMultiLine(fn), {
							id: this.id,
							classid: this.OfficeViewerClassID(),
							width: OV2.isClear(width) ? 650 : width,
							height: OV2.isClear(height) ? 450 : height,
							version: OV2.isClear(justep.doc._ocx_version2) ? 0 : justep.doc._ocx_version2,
			...
			function CheckViewer() {
				return this.Assert(
					this.IsOfficeViewer(),
					OV2.substitute("The {nodeName}[{id}] is not OfficeViewer.", {nodeName: this.nodeName, id: this.id}));
			}
			...
			function CheckInstalled(programID) {
				return this.CheckViewer() && this.Assert(
							...
							OV2.substitute("The computer hasn't installed Microsoft {pid}.", {pid: programID.replace(/MS|\.\w*/, "")}));
			}
			/**
			...
			function WordGetRevisionJSON() {
				if (this.CheckViewer() && this.IsWordOpened()) {
					var result = OV2.tryThese(this, function(source){
						var r = [];
			...
			function WordGetRevisionInfo(json, template, textLength, truncation) {
				var revisions = OV2.tryThese(this, function(source){
					return OV2.isClear(json) ?
							OV2.JSON.parse(source.WordGetRevisionJSON()) :
						(OV2.isString(json) && OV2.isJSON(json)) ?
								OV2.JSON.parse(json) :
							json;
				}, function(source){
					return json;
				...
				if (OV2.isArray(revisions)) {
					...
					for (var i = 0, length = revisions.length; i < length; i++) {
						...
						result += OV2.substitute(template, {
							Author: revision.Author,
							Date: OV2.trim(revision.Date.replace(/\"|[T|Z]/g, ' ')),
							Index: revision.Index,
							Text: OV2.truncate(revision.Text.replace(/\s+/g, ' '), textLength, truncation),
		...
		/**
		 * 历史修订模板
		 * OV2.RevisionTemplate.defaultHTMLContent 默认的HTML格式修订存储模板
		 */
...
justep.doc.Dialog = {
		...
		getOfficeEditorUrl:function(customUrl){
			var url = justep.Request.addBsessionid(window.location.protocol+"//"+window.location.host+justep.Request.convertURL((customUrl || "/UI/system/service/doc/office/officeEditorDialog.w")+"?process=/SA/doc/system/systemProcess&activity=mainActivity"));
			return url;
		},
		openOfficeDialog : function(callback, OVP, caller) {
			...
			if(!self.officeEditor){
				var option = {
					url:justep.doc.Dialog.getOfficeEditorUrl(caller ? caller.officeEditorUrl : ""),
					modal:"true",
					resizable:"true",
					status:"fullscreen"
				};
			...
			if(self.officeEditor){
				...
				var eventID = self.officeEditor.attachEvent("onReceive", callback,self);
				self.officeEditor.eventID = eventID;
				self.officeEditor.open({data:JSON.stringify(OVP)});
			}
		}
};
...
justep.doc.InnerUtils = {
	...
	queryDoc : function(docID, docPath, pattern, orderBy, custom , single) {
	 ...
	 getUploader2: function(param) {
		...
		//position: absolute; background-color: transparent; ; width: 54px; height: 15px; color: rgb(51, 102, 255); text-decoration: none;
		var flashContainer = containerID+'flashWarpper';
		$('<div></div>').attr('id',flashContainer).css({'position':'absolute','top':'0px','background-color':'transparent','width':'100%','height':'100%','z-index':zIndex}).appendTo($('#' + containerID));
		...
		uploader.fileSelect = function(event) {
			...
			for (var p in uploader.fileList) {
				...
				if ((file.size > limitSize) && (limitSize > 0)) {
					alert(new justep.Message(justep.Message.JUSTEP232105,justep.doc.InnerUtils.formatSize(limitSize)).getMessage());
			...
			try{				
				var host = justep.doc.InnerUtils.getdocServerAction(uploader.docPath, "/repository/file/cache/upload");
	...
	getIOSUploader : function(param){
		...
		$(button).bind('click',function() {
				...
				var uploadCallback = function(response, fileName) {
					...
					var size = $(file).attr("fileSize");
					onCompleteCallBack.call(caller, fileName, kind, size,
							cacheName);
					if(onCompleteCallBack){
						onCompleteCallBack.call(caller,docName,this,response);
					}
	...
	*/
	...
	browseDocByFileID : function(docPath, docName, fileID, versionID, partType, programID, isPrint){
		...
		else if(justep.doc._read_file_type2 && justep.doc._read_file_type2.indexOf((String(/\.[^\.]+$/.exec(docName)).toLowerCase())) >= 0){
	...
	execOfficeAction : function(id,param){
		if(id=='officeAction'){
			if(param){
				...
				justep.doc.InnerUtils.afterOfficeViewerDialogSelect.call(justep.doc.InnerUtils.caller, data);
			}else{
				//点编辑进去，点关闭出来需要解锁
				justep.doc.InnerUtils.afterOfficeViewerDialogSelect.call(justep.doc.InnerUtils.caller, "");
			}
			...
			if(justep.Browser.IE){
				if(!document.getElementById("justepDoc_ocx")){							  	
					$('<div style="height:0px;weight:0px;display:none;"></div>').html('<object id="justepDoc_ocx" classid="clsid:4771E057-4202-4F93-8F73-4C6654A9573D" width="0" height="0"' +
							'codebase='+justep.Request.convertURL("/UI/system/service/doc/office/office.cab", true)+'>' +
	...
	getUploadProgressDialog:function(){
		...
		if(!this.uploadProgressDialog){ 
			this.uploadProgressDialog = new justep.Dialog("docUploadProgressDialog", "正在上传...",true,null,550,215,null,null,function(){
				...
				self.uploadPprogressTable = new justep.doc.InnerUtils.ProgressTable({renderTo:"progressTable",width:"550px",columns:[
			...
			if(this.uploadPprogressTable){
			    ...
			    for (i = 0; i < data.length; i++) {
				   ...
				   if(!this.uploadPprogressTable.dm){
					...
					record.cancal = justep.doc.InnerUtils.ProgressTable.createCancalButton(record.id);
...
justep.doc.InnerUtils.ProgressTable.prototype.setValue = function(idx,fieldName,value){
...
justep.doc.InnerUtils.ProgressTable.prototype.deleteByField = function(fieldName,value){ 
	var idx = -1;
	for(var i=0;i<this.dm.length;i++){
		if(this.dm[i][fieldName]== value){
			idx = i;
...
justep.doc.InnerUtils.ProgressTable.createCancalButton = function (id) {
    return "<img id=" + id + " src='" + justep.Request.convertURL('/UI/system/images/doc/deletefile.gif',true) + "' onclick='justep.doc.InnerUtils.ProgressTable.cancal(this.id);' alt='取消上传'/>"; 
};
...
justep.doc.InnerUtils.ProgressTable.cancal = function(id){
	if(justep.doc.InnerUtils.uploadPprogressTable){
		...
		--uploader.allCount;
		if(uploader.allCount==0) justep.doc.InnerUtils.getUploadProgressDialog().close();
