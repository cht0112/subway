...
bizActivity9.mdDefaultLoad = function(event){
	//检测申请信息
	...
	var reccurrenceScheme = justep.xbl("recurrenceTestScheme");
	reccurrenceScheme.setFilter("filter0", "RECURRENCE_TEST_SCHEME.APPLICATION_NO="+justep.Context.getProcessData1()+" and RECURRENCE_TEST_SCHEME.IS_USED=1");
	reccurrenceScheme.loadData();
	
	var schemeId = reccurrenceScheme.getValue("TEST_SCHEME_ID");
	pData.setValue("tESTSCHEMEID",schemeId);
};	
...
bizActivity9.tabPage8_9Select = function(event){
	...
	if(projectData.getCount() > 0) {
		...
		var reccurrenceScheme = justep.xbl("recurrenceTestScheme");
		reccurrenceScheme.setFilter("filter0", "RECURRENCE_TEST_SCHEME.APPLICATION_NO="+no+" and RECURRENCE_TEST_SCHEME.IS_USED=1");
		reccurrenceScheme.refreshData();
		reccurrenceScheme.loadData();
		var schemeId = reccurrenceScheme.getValue("TEST_SCHEME_ID");
		...
		var recurrenceSchemeD = justep.xbl("recurrenceTestScheme");
	...
	var smeId = recurrenceSchemeD.getValue("TEST_SCHEME_ID",recurrenceSchemeD.getCurrentID());
				
	var schemeValiId = recurrenceSchemeD.getValue("TEST_SCHEME_ID");
	if(recurrenceSchemeD.getCount() > 0) {
		var caseHouseData = justep.xbl("caseHouseData");
		caseHouseData.setFilter("filter0", "CASE_HOUSE.FAPPLICATIONNO='"+justep.Context.getProcessData1()+"' and CASE_HOUSE.SCHEMEID="+schemeValiId);
...
*/
bizActivity9.vDataValueChanged = function(event){
...
bizActivity9.trigger9_1Click = function(event){
	...
	var reccurrenceScheme = justep.xbl("recurrenceTestScheme");
	reccurrenceScheme.setFilter("filter0", "RECURRENCE_TEST_SCHEME.APPLICATION_NO="+applicationNO+" and RECURRENCE_TEST_SCHEME.IS_USED=1");
	reccurrenceScheme.refreshData();
	reccurrenceScheme.loadData();
	var schemeId = reccurrenceScheme.getValue("TEST_SCHEME_ID");
	...
	param.setString("modeCode", "huigui");
...
bizActivity9.tabPage1_2Select = function(event){
...
*/
bizActivity9.taskDataValueChanging = function(event){
...
bizActivity9.tabPage1_8Select = function(event){
...
bizActivity9.insertTriggerClick = function(event){
...
*/
bizActivity9.hrDialogReceive = function(event){
...
*/
bizActivity9.hrDialogClose = function(event){
var qData = justep.xbl("projectMemberData");
	qData.setFilter("filter", "TEST_PROJECT_INFO.aPPLICATIONNO="+justep.Context.getProcessData1());
	qData.loadData();
	qData.refreshData();	
};

bizActivity9.triggerOutClick = function(event){
...
bizActivity9.triggerinputClick = function(event){
    var applicationNo = justep.Context.getProcessData1();
	var ams = $('#input17_3').val();
	var ame = $('#input18_3').val();
	var pms = $('#input19_3').val();
...
description: <b>[回调型事件]</b>关闭时触发
*/
bizActivity9.windowDialog2Close = function(event){
