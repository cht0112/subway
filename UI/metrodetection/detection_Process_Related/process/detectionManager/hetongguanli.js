...
hetongguanli.tabPage4_1Select = function(event) {
...
hetongguanli.tabPage1_6Select = function(event) {
	var CONTRACTData = justep.xbl("CONTRACTData");
	//CONTRACTData.refreshData();
};

hetongguanli.tabPage2_6Select = function(event) {

	var SAMPLE_DEVICEData = justep.xbl("SAMPLE_DEVICEData");
	SAMPLE_DEVICEData.setFilter("filter0", "SAMPLE_DEVICE_INFO.aPPLICATIONNO="
			+ justep.xbl("CONTRACTData").getValue("aPPLICATIONNO"));
	...
	if (b == "") {
		...
		// $("#insertTrigger").attr("onclick",
		// "hetongguanli.insertTriggerClick1(event)");
	}
};
 ...
 * 
 * @event description: <b>[回调型事件]</b>关闭时触发
 */
hetongguanli.windowDialog2_1Close = function(event) {
	var mData = justep.xbl("CONTRACTData");
	mData.refreshData();
};

hetongguanli.insertTriggerClick1 = function(event) {
 ...
 * @event description: <b>[回调型事件]</b>关闭时触发
 */
hetongguanli.windowDialog3_1Close = function(event) {
	var mData = justep.xbl("SAMPLE_DEVICEData");
	mData.refreshData();
};
// 双击打开修改
hetongguanli.grid4_6RowDblClick = function(event) {
...
hetongguanli.mdDefaultXBLLoaded = function(event) {
	...
	var CONTRACTData = justep.xbl("CONTRACTData");
	CONTRACTData.setFilter("filter0", "TEST_CONTRACT_INFO.aPPLICATIONNO ="
			+ justep.Context.getProcessData1());
	CONTRACTData.refreshData();
	if (CONTRACTData.getCount() == 0) {
		...
		CONTRACTData
				.setValue("aPPLICATIONNO", justep.Context.getProcessData1());

		// var a = justep.xbl("dataMain");
		// var aSSIGNEDMANUFACTUREID=
		// dataMain.getValue("aSSIGNEDMANUFACTUREID");
		// var CONTRACTData = justep.xbl("CONTRACTData");
...
// 1
hetongguanli.removeTrigger1Click = function(event) {
...
hetongguanli.gridSelect1_17Dropdown = function(event){
