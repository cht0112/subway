...
schemeProcessQueryDetail.model1ModelConstructDone = function(event){
	...
	//if (operator == "view") {
		...
		testSchemeBaseInfoD.setFilter("testApplicationDataFilter","TEST_SCHEME_BASE_INFO ="+justep.Context.getProcessData1());
...
///	var id = justep.Request.URLParams.id;
	var data = justep.xbl('dataMain');
	data.setFilter("filter0", "SA_Task.sData1='"+justep.Context.getProcessData1()+"' AND SA_Task.sParent is null AND SA_Task.sTypeName='检测方案基本信息'");
	data.refreshData();
