...
mainActivity3.model1Load = function(event){
	...
	if(labData.getCount() > 0) {
		...
		progressData.setFilter("filter0", "JINDU_VIEW.TOOLNO="+roomId);
		progressData.refreshData();
		if(progressData.getCount() > 0) {
		/*var pId = progressData.getValue("FPARENTID", progressData.getID(0));
		//alert(pId);
		progressData.setFilter("filter0", "JINDU_VIEW.TOOLNO="+roomId+" or JINDU_VIEW='"+pId+"' or JINDU_VIEW.FPARENTID IS NULL");
		progressData.refreshData();
		...
		progressData.setFilter("filter0", "JINDU_VIEW.TOOLNO="+roomId+" or JINDU_VIEW in ( "+arrayObj+") or JINDU_VIEW.FPARENTID IS NULL or JINDU_VIEW in ( "+arrayObj2+")");
		progressData.refreshData();
		progressData.expandAll();
	}
	}
...
mainActivity3.grid1RowClick = function(event){
	...
	progressData.setFilter("filter0", "JINDU_VIEW.TOOLNO="+rowId);
	progressData.refreshData();
	if(progressData.getCount() > 0) {
		/*var pId = progressData.getValue("FPARENTID", progressData.getID(0));
		//alert(pId);
		progressData.setFilter("filter0", "JINDU_VIEW.TOOLNO="+rowId+" or JINDU_VIEW='"+pId+"' or JINDU_VIEW.FPARENTID IS NULL");
		progressData.refreshData();
		progressData.expandAll();*/
		...
		progressData.setFilter("filter0", "JINDU_VIEW.TOOLNO="+event.rowID+" or JINDU_VIEW in ( "+arrayObj+") or JINDU_VIEW.FPARENTID IS NULL or JINDU_VIEW in ( "+arrayObj2+")");
		progressData.refreshData();
		progressData.expandAll();
