...
mainActivity1.model1Load = function(event){
...
mainActivity1.gridRowDblClick = function(event){
	var process = justep.Context.getCurrentProcess();
	var activity = justep.Context.getCurrentActivity();
	var operator = "view";
	var url = "/metrodetection/system_resource/process/newchangeQuery/complaintProcessQueryDetail.w?process="
			+ process
			+ "&activity="
			+ activity
			+ "&operator="
			+ operator
			+ "&id=" + event.rowId;
	justep.Portal.openWindow("检测计划流程查询详细", url);

};

mainActivity1.grid_sNameRender = function(event){
	if (event.value == "") {
		event.value = "&#160;&#160;&#160;&#160;&#160;";
	}
...
mainActivity1.gridSelect1Closeup = function(event){
	var dFilter = justep.xbl("custom_filter");
	var main = justep.xbl("main");
...
mainActivity1.trgSearchClick = function(event){
	justep.xbl("main").refreshData();
};

mainActivity1.gridSelect2Closeup = function(event){
	var dFilter = justep.xbl("custom_filter");
