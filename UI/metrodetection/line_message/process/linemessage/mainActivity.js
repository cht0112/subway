...
mainActivity.model1Load = function(event){
	...
	if(labData.getCount()>0){
		...
		progressData.setFilter("filter0", "JINDU_VIEW_NEW.LINEID="+lineID);
		progressData.refreshData();
		if(progressData.getCount() > 0) {
			/*var pId = progressData.getValue("FPARENTID", progressData.getID(0));
			//alert(pId);
			progressData.setFilter("filter0", "JINDU_VIEW_NEW.LINEID="+lineID+" or JINDU_VIEW_NEW='"+pId+"' or JINDU_VIEW_NEW.FPARENTID IS NULL");
			progressData.refreshData();
			...
			progressData.setFilter("filter0", "JINDU_VIEW_NEW.LINEID="+lineID+" or JINDU_VIEW_NEW in ( "+arrayObj+") or JINDU_VIEW_NEW.FPARENTID IS NULL");
			progressData.refreshData();
			progressData.expandAll();
		}
	}
