...
public class OPMProcedure {
	//	public static Object login(String name, Date loginDate, String pwd, String lang) {
	...
	public static Table query(String concept, String idColumn, String select, String from, String condition, List<DataPermission> range,
			...
			Map<String, Object> variables, String dataModel, String fnModel, String manageCodes, boolean includeChildrenOfManage) {
		return OrgQuery.query(concept, idColumn, select, from, condition, range, filter, distinct, offset, limit, columns, orderBy, aggregate,
				aggregateColumns, variables, dataModel, fnModel, manageCodes, includeChildrenOfManage);
	}
	...
	private static Object login(String name, Date loginDate, String pwd, String lang, boolean checkPwd, String ip, Map<Object, Object> options) {
		...
		Utils.checkNotExist("RequestContext", requestContext);
		SessionContext sessionContext = requestContext.getSessionContext();
		Utils.checkNotExist("SessionContext", sessionContext);
		Person psn = OrgUtils.getPerson(name, pwd, checkPwd);
		Utils.check(psn != null, "未找到登录名为：", name, "的 人员！");

		Operator operator = new Operator(psn, loginDate, lang);
		sessionContext.put(OrgConstants.ORG_CACHE, operator);
		//		SystemCache systemCache = new SystemCache(loginDate, lang);
		//		sessionContext.put(OrgConstants.SYSTEM_CACHE, systemCache);

		List<Object> result = new ArrayList<Object>();
		result.add(Boolean.TRUE);
		result.add(operator.getID());
	...
	*/
	public static Document queryOrgsByResource(String resourceID, String typeID, String process, String activity, String action){
		...
		String query = "select t.sOrgFID, t.sOrgFName, t.sOrgName from SA_ResourceControl t where t.sResourceID=:resource and " +
				"t.sTypeID=:type";
	...
	public static void saveResourceControl2(String resourceID, String typeID, String typeName, 
			Map<String, String> newOrgs, String process, String activity, String action){
		Utils.check(Utils.isNotEmptyString(resourceID), CommonMessages.class, CommonMessages.PARAM_NULL1, "resourceID");
		Utils.check(Utils.isNotEmptyString(typeID), CommonMessages.class, CommonMessages.PARAM_NULL1, "typeID");
		...
		for (String fid : newOrgs.keySet()){
			...
			String insert = "insert into SA_ResourceControl t (t, t.sResourceID, t.sTypeID, t.sTypeName, " +
					"t.sOrgFID, t.sOrgFName, t.sOrgID, t.sOrgName, " +
					"t.sCreatorFID, t.sCreatorFName, t.sCreatorID, t.sCreatorName," +
					"t.sCreateTime, t.version) values (:id, :resourceID, :typeID, :typeName, " +
					":orgFID, :orgFName, :orgID, :orgName," +
					":creatorFID, :creatorFName, :creatorID, :creatorName, :createTime, 0)";
	...
	/*
	 常用组树形数据格式
		虚根				_group_: 常用组													
		自定义组				groupID: groupName											
		组织						/_group_/groupID/memberID/orgID: orgName					
	 */
	...
	private static Table queryOrgCommonGroupRoot(String concept, String idColumn, String select, String from, String condition, List<DataPermission> range,
			String filter, Boolean distinct, int offset, int limit, String columns, String orderBy, String aggregate, String aggregateColumns,
		...
		// 获取常用组
		String groupSQL = "select customGroup as sID, customGroup.sName, customGroup.sTypeID, customGroup.sSequence " +
				"	from SA_OPCustomGroup customGroup " +
				"		optional join SA_ResourceControl range1 " +
				"			on range1.sTypeID = 'customGroup_range' and range1.sResourceID = customGroup " +
				"	where ((customGroup.sTypeID = 'private' and customGroup.sCreatorID = :currentPersonID()) " +
				"			or (customGroup.sTypeID = 'public' and (range1 is null or (" + publicRange + ")))) " +
				"	group by customGroup, customGroup.sName, customGroup.sTypeID, customGroup.sSequence " +
				"	order by customGroup.sTypeID, customGroup.sSequence ";
		...
		select = select + ", member.sResourceID as groupID, member as memberID ";
		from = from + " join SA_ResourceControl member on SA_OPOrg.sFID = member.sOrgFID "
			+ " and member.sTypeID = 'customGroup_member' " 
			+ "	and member.sResourceID in ('" + OpmUtils.arrayToString(groups.toArray(), "%s", "','") + "')";
		Table orgTable = BizData.query(concept, idColumn, select, from, condition, range, filter, distinct, offset, limit, columns, orderBy, aggregate, aggregateColumns, variables, dataModel, fnModel);
	...
	public static Table queryOrgCommonGroup(String concept, String idColumn, String select, String from, String condition, List<DataPermission> range,
			String filter, Boolean distinct, int offset, int limit, String columns, String orderBy, String aggregate, String aggregateColumns,
			Map<String, Object> variables, String dataModel, String fnModel, String parentID) {
		if (Utils.isEmptyString(parentID)) {
			return queryOrgCommonGroupRoot(concept, idColumn, select, from, condition, range,
					filter, distinct, offset, limit, columns, orderBy, aggregate, aggregateColumns,
					variables, dataModel, fnModel);
		}
