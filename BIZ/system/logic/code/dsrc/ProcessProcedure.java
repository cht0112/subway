...
public class ProcessProcedure {
	...
	*/
	...
	public static Document startProcess(String processFullName, Map<String, Object> relations, ProcessControl control, List<String> tokens, String executor, String creator) {
		ProcessEngine engine = (ProcessEngine) ProcessUtils.getProcessContext();
	...
	public static Document advanceProcess(String task, ProcessControl control) {
		ProcessEngine engine = (ProcessEngine) ProcessUtils.getProcessContext();
		engine.advance();
		engine.commit();
	...
	public static Document backProcess(String task, ProcessControl control) {
		ProcessEngine engine = (ProcessEngine) ProcessUtils.getProcessContext();
		engine.back();
		engine.commit();
	...
	*/
	...
	public static Document executeTask(String task, String executor) {
		Utils.check(Utils.isNotEmptyString(task), CommonMessages.class, CommonMessages.PARAM_NULL1, ProcessConstants.TASK_PARAMETER); 
		...
		Document result = DocumentHelper.createDocument();
		result.addElement("executor").addText(fid);
		return result;
	}
	...
	public static Document queryTaskData(String task) {
		Map<String, String> data = getTaskData(task, true);
		Document result = DocumentHelper.createDocument();
		Element taskData = result.addElement("task-data");
		String sData1 = data.get("sData1");
		taskData.addElement("sData1").addText((sData1 == null) ? "" : sData1);

		String sData2 = data.get("sData2");
		taskData.addElement("sData2").addText((sData2 == null) ? "" : sData2);

		String sData3 = data.get("sData3");
		taskData.addElement("sData3").addText((sData3 == null) ? "" : sData3);

		String sData4 = data.get("sData4");
		taskData.addElement("sData4").addText((sData4 == null) ? "" : sData4);
	...
	public static String getProcessChartByData(String process, String data, String ext){
		...
		if (pis.isEmpty()){
			return ProcessChartEngine.getProcessChartByProcess(process, ext); 
		}else{
