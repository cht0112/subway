...
public class PublishKnowledgeProcess {
	private static final String DATA_MODEL = "/OA";

/*	public static void afterSaveKMKnowledgeAction() {
		System.out.println(ActionUtils.getRequestContext().getActionContext().getParameter("otherFolderIDs"));
	}*/

	public static void publishKnowledgeProcessAfterAbort() throws Exception {
		Connection conn = ModelUtils.getConnection(DATA_MODEL);
	...
	public static void publishKnowledgeProcessAfterAdvance() throws Exception {
	...
	private static void updateKWReleaseStatus(Connection conn, int status)
		...
		String fReleaseDeptID = SysUtils.getCurrentDeptID() != null ? SysUtils
				.getCurrentDeptID() : SysUtils.getCurrentOgnID();
		String fReleaseDeptName = SysUtils.getCurrentDeptName() != null ? SysUtils
				.getCurrentDeptName()
		...
		String sql = String
				.format(
						"update OA_KM_Knowledge "
								...
								+ " where fid = '%s'", fReleaseOgnID,
						fReleaseOgnName, fReleaseDeptID, fReleaseDeptName,
						fReleasePsnID, fReleasePsnName, fReleasePsnFID,
						fReleasePsnFName, currentDate, fReleaseStatus,
						fReleaseStatusName, kwid);
	...
	public static void chargeApvActivityAfterAdvance() throws Exception {
		...
		try {
			...
			ProcessControl flowControl = (ProcessControl) ModelUtils
					.getRequestContext().getActionContext().getParameter(
							"control");
