...
public class EncryptModel {
	...
	static {
		//加载dll
		String dll = SystemUtils.normalizeLib(SystemUtils.getAppHome() + "/lib", JNI);
		System.load(dll);
	}
	...
	private static void copyDirectiory(String sourceDir, String targetDir) throws IOException {
		...
		boolean isCacheDir = sourceDir.endsWith(".cache");
	...
	public void encryptDir(String dir, String key) throws IOException {
		...
		for (File f : fDir.listFiles()) {
			...
			if (f.isFile()) {
				if (fileName.endsWith(".m") || (-1 < fileName.indexOf("/.release/") && !fileName.endsWith(".sign"))) {
	...
	/*
	 * 模型根据开发者证书签名
	 * model支持格式：/BIZ/demo、/UI/demo、/demo
	 */
	public void encrypt(String model, String key, boolean isCompile) throws IOException {
		if (null == model || "".equals(model) || !model.startsWith("/"))
			throw new RuntimeException("输入的模块路径不正确，支持格式：/BIZ/demo、/UI/demo、/demo");
	...
	protected void compile(String model) {
		...
		if (s.startsWith("/biz/")) {
			compileBIZ(model);
		} else if (s.startsWith("/ui/") || s.startsWith("/mobileui/")) {
			compileUI(model);
		} else {
	...
	protected void compileUI(String model) {
		...
		String cmd = justep_home + "/jdk/bin/java -Xms64m -Xmx512m -classpath %CLASSPATH% com.justep.tools.CompileUI %UI% %MOBILE_UI% -w";
	...
	protected void compileBIZ(String model) {
		...
		String cmd = justep_home + "/jdk/bin/java -Xbootclasspath/a:%CLASSPATH% -jar %MODEL_LIB%/compileBIZ.jar %BIZ% %BIZ_LIB% %MODEL_LIB%";
